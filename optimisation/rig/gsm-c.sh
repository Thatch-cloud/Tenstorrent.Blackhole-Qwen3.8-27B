...
echo "### $TAG gdn_bf16=$BF8 load=[$(cut -d' ' -f1-3 /proc/loadavg)] start=$(date -Is)"
...
docker run -d --name epserve -p 8001:8000 -w /opt/vllm-tt-plugin \
  ...
  -e QWEN_GDN_FUSED_DECODE=1 -e QWEN_SDPA_BF8=1 -e QWEN35_GDN_DECODE_BF16="$BF8" -e QWEN35_GDN_STATE_BF16="$BF8" \
