#!/usr/bin/env bash
# Local ttsim run of gdn-prefill-conv.py (lever #2: gdn_prefill_conv_exact vs the served FIR).
# Mirrors run-gdn-commit.sh: the 1x2 P300 simulator, slow dispatch, the /opt/ttsim tt-metal tree.
set -euo pipefail
...
export QWEN_SIM_REPORT="$SIM_ROOT/results/$RUN_ID-gdn-prefill-conv.json"
printf 'report=%s\n' "$QWEN_SIM_REPORT"
cd "$SIM_ROOT"
status=0
timeout -k 10 "${KERNEL_TIMEOUT:-3600}" "$SIM_ROOT/venv/bin/python" "$SCRIPT_DIR/gdn-prefill-conv.py" "$@" 2>&1 \
  | sed -E '/\| +info +\| +Metal +\| (DFB size:|Writing DFB config)/d' | tee "$SIM_ROOT/results/$RUN_ID-gdn-prefill-conv.log" || status=$?
printf '%s\n' "$status" > "$SIM_ROOT/results/$RUN_ID-gdn-prefill-conv.exit-status"
exit "$status"
