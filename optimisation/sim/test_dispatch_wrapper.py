...
class DispatchWrapperTests(unittest.TestCase):
    ...
    def test_full_history_attention_route_preserves_terminal_status(self):
        source = Path(__file__).with_name('run-dispatch-probe.sh').read_text()
        code, output, statuses = self.run_edit_race(source, 'dspark-full-attention-probe')
        self.assertEqual(code, 0, output)
    ...
    def test_dspark_projection_route_preserves_terminal_status(self):
        source = Path(__file__).with_name('run-dispatch-probe.sh').read_text()
        for probe in ('dspark-projection-probe','dspark-norm-precision-probe','dspark-layer-probe','dspark-residual-probe'):
