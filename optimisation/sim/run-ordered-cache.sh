...
export QWEN_SIM_REPORT="$SIM_ROOT/results/$RUN_ID-ordered-cache.json"
printf 'report=%s\n' "$QWEN_SIM_REPORT"
cd "$SIM_ROOT"
status=0
timeout -k 10 "${KERNEL_TIMEOUT:-300}" "$SIM_ROOT/venv/bin/python" "$SCRIPT_DIR/ordered-cache.py" "$@" 2>&1 | sed -E '/\| +info +\| +Metal +\| (DFB size:|Writing DFB config)/d' | tee "$SIM_ROOT/results/$RUN_ID-ordered-cache.log" || status=$?
printf '%s\n' "$status" > "$SIM_ROOT/results/$RUN_ID-ordered-cache.exit-status"
exit "$status"
