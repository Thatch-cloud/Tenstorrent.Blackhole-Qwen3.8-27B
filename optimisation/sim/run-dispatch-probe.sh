...
[[ "$PROBE" = draft-row-sum-probe || "$PROBE" = learned-attention-probe || "$PROBE" = draft-attention-probe || "$PROBE" = learned-convolution-probe || "$PROBE" = draft-convolution-probe || "$PROBE" = dispatch-probe || "$PROBE" = feature-trace-probe || "$PROBE" = prefill-feature-probe || "$PROBE" = feature-projection-probe || "$PROBE" = feature-norm-probe ]]
