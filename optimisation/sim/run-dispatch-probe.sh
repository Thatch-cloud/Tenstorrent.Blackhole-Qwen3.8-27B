...
[[ "$PROBE" = dspark-rotary-probe || "$PROBE" = dspark-markov-rounding-probe || "$PROBE" = dspark-markov-probe || "$PROBE" = proposal-native-attention-probe || "$PROBE" = tensix-mlp-view-probe || "$PROBE" = draft-live-attention-probe || "$PROBE" = tensix-weight-stream-probe || "$PROBE" = tensix-stream-projection-probe || "$PROBE" = tensix-stream-mlp-probe ]] ||
