...
[[ "$PROBE" = draft-convolution-fused-probe || "$PROBE" = dflash-proposal-trace-probe || "$PROBE" = native-draft-operands-probe || "$PROBE" = draft-head-layout-probe || "$PROBE" = draft-key-concat-probe || "$PROBE" = dflash-history-probe ]] ||
