...
void kernel_main() {
    ...
    */
    ...
    // (factory F21: +4, after F6's four). Under it this non-causal kernel takes the causal block's cur_pos read
    // below: each entry's E - 1 from the cur_pos tensor, or UINT32_MAX to skip the entry.
    constexpr bool runtime_extent = get_compile_time_arg_val(qwen_cta + 4) == 1;
