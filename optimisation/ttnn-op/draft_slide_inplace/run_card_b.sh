#!/usr/bin/env bash
# Run draft_slide_inplace_card_b.py (M-F0, round-fence plan S0.1: the served draft K/V slide kernel out
# of place against IN PLACE, byte for byte, plus generic_op's aliased io list, the program cache and the
# 10-bank timings) on the qualification card (QUAL_CARD, default card B), in the serving image. NOT a CI
# workflow: invoke by hand on the rig.
#
#   bash run_card_b.sh                 # the full matrix, the cache and trace checks, and timing
#   WATCHER=1 bash run_card_b.sh       # THE FIRST HARDWARE PASS: TT_METAL_WATCHER=5 (NoC sanitiser,
#                                      # waypoints), the quick matrix, no timing, a per-call watchdog
#                                      # (WATCHDOG_S, default 120 s) that os._exit(3)s the harness, and a
#                                      # 900 s container timeout
#
# The served kernel is the IMAGE's: /experiment-scripts/ci/draft_kv_slide.{py,cpp}, from serving-bundle
# 35489235797 in every fast-serving image (they are in neither image copy list); that .cpp is the
# direct-DMA kernel (1679bbd7). This checkout's two qualified kernels are mounted beside it, file by file,
# into /bench/slide: scripts/ci/draft_kv_slide.cpp (the scalar kernel, bc45d472) as scalar/, and
# scripts/ci/draft_kv_slide_direct.cpp as direct/ (under the served file name, as the --direct-dma stage
# stages it; the harness runs it only if its bytes differ from the image's), plus the checkout driver for
# the hash comparison. No image build, no .so, no weights: the kernels JIT into a fresh per-run kernel
# cache; random bf16 banks of the real shapes.
#
# Env: REPO (this checkout; default three levels up), IMAGE (default image P2, the v191-v193 image),
# RESULTS (~/kwork64/slide/<card tag>: card-b, card-m, card-a), CARD_B_ARGS (extra harness args, appended
# last, e.g. "--sections forms,cases --kernels served"), WATCHER=1, WATCHDOG_S, QUAL_CARD (the target
# board id under /dev/tenstorrent/by-id; default card B, blackhole-F36F768B9A5CAFA0; card M or card A,
...
IMAGE=${IMAGE:-sha256:463964d577321f2cebd5d29422edcaf848ade3ba4c5260c890497b6b2accfe98}
...
# <<< qual_card.sh
qual_card_select
R=${RESULTS:-$HOME/kwork64/slide/$QUAL_TAG}
name=qwen-slide-$QUAL_TAG
...
SM=()
for pair in draft_kv_slide.cpp:scalar/draft_kv_slide.cpp draft_kv_slide_direct.cpp:direct/draft_kv_slide.cpp \
    draft_kv_slide.py:draft_kv_slide.py; do
  src=$REPO/scripts/ci/${pair%%:*}
  test -s "$src" || { echo "$src missing" >&2; exit 1; }
  SM+=(--mount "type=bind,src=$src,dst=/bench/slide/${pair#*:},readonly")
done
test_file="$here/draft_slide_inplace_card_b.py"
test -s "$test_file" || { echo "$test_file missing" >&2; exit 1; }

mkdir -p "$R" "$R/kcache-$stamp"
chmod 0777 "$R" "$R/kcache-$stamp"
args=(--out "/results/slide-$stamp.json" --checkout-dir /bench/slide)
WM=()
if [ "${WATCHER:-}" = "1" ]; then
  timeout_s=900
...
echo "### slide $stamp card=$QUAL_CARD ($QUAL_TAG) node=$node image=${IMAGE:7:12}"
...
timeout -k 30 "$timeout_s" docker run --rm --name "$name" --network none \
  ...
  "${SM[@]}" \
  --mount "type=bind,src=$test_file,dst=/bench/draft_slide_inplace_card_b.py,readonly" \
  ...
  --entrypoint python3 "$IMAGE" -B /bench/draft_slide_inplace_card_b.py "${args[@]}" "${extra[@]}" \
  2>&1 | tee "$R/slide-$stamp.log"
status=${PIPESTATUS[0]}
echo "### exit $status; report $R/slide-$stamp.json"
hung=0
case "$status" in
  3|124|137) hung=1 ;;
  1) grep -qF 'Timeout (' "$R/slide-$stamp.log" && hung=1 ;;   # the faulthandler backstop (GIL held)
esac
if [ "$hung" = 1 ]; then
