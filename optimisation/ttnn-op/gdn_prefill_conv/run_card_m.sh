#!/usr/bin/env bash
# Run gdn_prefill_conv_card_m.py (lever #2: the GDN prefill conv op) on the qualification card
# (QUAL_CARD, default card B), in the serving image.
#
...
# RESULTS (~/kwork64/pcx/<card tag>: card-b, card-m, card-a), CARD_M_ARGS (extra args, appended
# last, e.g. "--ts 32,64"), WATCHER=1, WATCHDOG_S, PROFILE=1, MODELS=1 (MODEL_DIR: the HF snapshot,
# default the arm's), QUAL_CARD (the target board id under /dev/tenstorrent/by-id; default card B,
...
# <<< qual_card.sh
qual_card_select
R=${RESULTS:-$HOME/kwork64/pcx/$QUAL_TAG}
name=qwen-pcx-$QUAL_TAG
...
echo "### pcx $stamp card=$QUAL_CARD ($QUAL_TAG) node=$node image=${IMAGE:7:12} op=${op_files[*]}"
