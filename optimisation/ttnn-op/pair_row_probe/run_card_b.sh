#!/usr/bin/env bash
# P1 for the pair drafter's row-1 fix (QWEN_FAST_PAIR_ROW_EXACT, scripts/ci/pair_row_exact.py): run
# probe_pair_row_card_b.py on the qualification card (QUAL_CARD, default card B), in the gate's image. The served
# graft is mounted exactly as lever_n_m3native_run_arm.sh mounts it for the v212-v217 argv (KOPGRAFT64 with
# M3NATIVE_SDPA_PF=1): _ttnn.so, _ttnncpp.so (both paths), attn_prep, nlp_concat_heads_decode, sdpa_decode and
# sdpa. The draft SDPA's factory is in that _ttnncpp.so and its kernels are JIT-compiled from that sdpa/ (the
# image's own plus the chain reader). The served modules the cases call - pair_row_exact, draft_attention,
# dflash_batched_mask and draft_shared_head - are this checkout's, mounted beside the harness, so the probe needs
# no new image. No build, no weights. NOT a CI workflow: invoke it by hand on the rig.
#
#   bash run_card_b.sh                 # the probe: C0-C8 and P, then eager and traced timing (about 30 min)
#   WATCHER=1 bash run_card_b.sh       # THE FIRST HARDWARE PASS (the folded 32-head call and the per-head mask
#                                      # were never run): TT_METAL_WATCHER=5 (NoC sanitiser, waypoints), seed 0,
#                                      # normal and partner100, C4 at 1 and 65 chunks, no head, no timing, a per-call
#                                      # watchdog (WATCHDOG_S, default 120 s), a 1,800 s container timeout; the
#                                      # watcher's own log lands in $RESULTS/watcher-<stamp>/
#   PAIR_ROW_DRY_RUN=1 bash run_card_b.sh
#                                      # print the launch argv and exit: no node resolution, no holder check, no
#                                      # docker, nothing launched (the graft is checked when it exists)
#
# Before anything is launched the graft must verify against its MANIFEST.sha256 and, when EXPECT_TTNNCPP_SHA256 is
# set, its _ttnncpp.so must be that. The harness checks the mapped binary again inside the container, records it,
# and refuses an SDPA source tree that is not the T16 admission's pinned one.
#
# Env: KOPGRAFT64 (default ~/opgraft-K64i, the v212-v217 graft; 'none' runs the image's own binary and op tree),
# IMAGE (default image P5, the v206-v217 gate image), RESULTS (~/kwork64/pairrow/<card tag>: card-b, card-m,
# card-a), CARD_B_ARGS (extra harness args, appended last, e.g. "--seeds 0 --regimes normal"),
# EXPECT_TTNNCPP_SHA256 (default unset: recorded, not required), WATCHER=1, WATCHDOG_S, PAIR_ROW_DRY_RUN=1, QUAL_CARD
...
IMAGE=${IMAGE:-sha256:0fd9ad1f14a4e5d3d4464be55465cb6bb8d52e1b533c430d1c2f7f219df2b0e8}
...
# <<< qual_card.sh
qual_card_select
R=${RESULTS:-$HOME/kwork64/pairrow/$QUAL_TAG}
OPS=/opt/tt-metal/ttnn/cpp/ttnn/operations
DRY=${PAIR_ROW_DRY_RUN:-0}
name=qwen-pairrow-$QUAL_TAG
stamp=$(date +%Y%m%dT%H%M%S)
timeout_s=3600   # C0-C8 over 3 seeds x 4 regimes, the head's 1.27 GB shard twice, then 5 variants x 2 scopes timed
...
HARNESS=("$here/probe_pair_row_card_b.py" "$ci/pair_row_exact.py" "$ci/draft_attention.py"
         "$ci/dflash_batched_mask.py" "$ci/draft_shared_head.py")
...
KM=()
if [ "$G" = none ]; then
  echo "### KOPGRAFT64=none: the image's own _ttnncpp.so and op tree (not what v212-v217 serve)"
else
  if [ "$DRY" = 1 ] && [ ! -e "$G" ]; then
...
WM=()
if [ "${WATCHER:-}" = "1" ]; then
  # One pass under the NoC sanitiser over the SDPA cases, the fixes and the row-local sweep; timing is meaningless
  # here. CARD_B_ARGS, appended last, can widen it.
  timeout_s=1800
  args+=(--seeds 0 --regimes normal,partner100 --leading 1,65 --head-dtypes "" --no-timing --watchdog "${WATCHDOG_S:-120}")
...
echo "### pairrow $stamp card=$QUAL_CARD ($QUAL_TAG) node=$node image=${IMAGE:7:12} graft=$G watcher=${WATCHER:-0}"
...
# Anchored: the summary line 'PAIR_ROW_PROBE_DONE ...' also starts with 'PAIR_ROW_PROBE'.
echo "### $(grep -E '^PAIR_ROW_PROBE ' "$log" | tail -1 || echo 'no PAIR_ROW_PROBE line')"
hung=0
case "$status" in
  3|124|137) hung=1 ;;
