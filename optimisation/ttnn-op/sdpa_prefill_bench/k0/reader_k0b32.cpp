...
// [QWEN-SDPA-K0] k0b32: K0 probe reader, NOT for serving (sdpa-prefill-share-spec.md 7.1).
// reader_interleaved.cpp (sha256 f97f5490...) + the K0 edits of
// optimisation/ttnn-op/sdpa_prefill_bench/make_k0_readers.py; mounted over the image reader.
// as k0b4 with a K/V read barrier every 32 tiles on those 16 cores (flag 0x2 cadence).
...
void kernel_main() {
    ...
    // 16 would-be G6 injectors; they read every K/V chunk with a barrier every 32 tiles, the rest
    // read only k_chunk 0 and 1 at the served barrier_threshold.
    const bool qwen_k0_full_kv = ((core_id / 8) % 6) == 0;
    const uint32_t qwen_k0_kv_bt = qwen_k0_full_kv ? 32u : barrier_threshold;
            ...
            for (uint32_t k_chunk = k_loop_start; (k_chunk * Sk_chunk_t) < q_high_idx; ++k_chunk) {
                ...
                if (should_receive) {
                    ...
                    if constexpr (is_chunked) {
                        ...
                        if (qwen_k0_full_kv || k_chunk < 2) {   // [QWEN-SDPA-K0] DRAM read: the served call
                            ...
                            // reserve then push dst_rows * dst_cols = Sk_chunk_t * DHt tiles over the stale slot.
                            cb_k.reserve_back(Sk_chunk_t * DHt);
                            cb_k.push_back(Sk_chunk_t * DHt);
                        }
                    } else {
                        if (should_forward) {
                            cb_k.reserve_back(k_chunk_tiles);
                ...
                if (should_receive) {
                    ...
                    if constexpr (is_chunked) {
                        ...
                        if (qwen_k0_full_kv || k_chunk < 2) {   // [QWEN-SDPA-K0] DRAM read: the served call
                            ...
                            // [QWEN-SDPA-K0] no DRAM read: the served call's own CB lifecycle only,
                            // reserve then push dst_rows * dst_cols = Sk_chunk_t * vDHt tiles over the stale slot.
                            cb_v.reserve_back(Sk_chunk_t * vDHt);
                            cb_v.push_back(Sk_chunk_t * vDHt);
                        }
                    } else {
                        if (should_forward) {
                            cb_v.reserve_back(v_chunk_tiles);
