...
The sources read are tt-metal 9f9cd4fd, the same bytes the serving image and K64i carry. The probe re-checks every
hash in the container.
...
- **The writer computes its own split from its own `cur_pos`** (`writer_decode_all.cpp:118-138`, `:146-183`). It
  ...
  - Above that, the stale writer's tree happens to be the live one. **K64j must change the writer**: the stock
    writer, used for flags 0x1-0x3, and the slice writer, used for 0x4.
- **Compute** (`sdpa_flash_decode.cpp:139-177`) reads `c_15` only under `is_causal`, so K64j must extend that
  branch too. The tail predicate `k_chunk == k_num_chunks - 1` then uses the runtime chunk count, which is right.
...
- **The skipped entry's output rows are never written.** They hold whatever the DRAM held. K64j's consumers
  must ignore an idle segment's rows; the row-independent consumers do.
...
- **NO-DECISION:** a failure (a wrong binary or wrong stock kernels, no compact scratch, a served mode that differs,
  a qwen program without its factory line, an error, the watchdog), or a dead liveness control.
...
**On CPU, from this directory** (Python 3.11; about 20 s, and the bash runner tests need Git Bash on Windows):
...
9. **The arm.**
   ...
   - The kernel-cache key must hash every `*qwen*.cpp` under the graft's `sdpa_decode`; today it hashes only the two
     stage-3 kernels.
