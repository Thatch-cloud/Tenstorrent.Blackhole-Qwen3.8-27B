...
ProgramDescriptor NLPConcatHeadsDecodeProgramFactory::create_descriptor(
    ...
    // head_size is both the byte size of one head row of the input shard and the tile-row stride of
    // the output shard: the output shard is (batch x head_dim) = ceil(batch/32) tile rows of
    // head_tiles tiles, tiles row-major. The reader relies on the second reading to step from one
    // 32-user batch tile to the next, so the two must stay the same quantity.
