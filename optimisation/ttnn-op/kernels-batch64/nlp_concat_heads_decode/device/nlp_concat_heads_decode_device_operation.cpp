...
void NLPConcatHeadsDecodeDeviceOperation::validate_on_program_cache_miss(
    ...
    // Batch (users) is the output's row axis, and the output is one 32-row tile per 32 users.
    // A batch that is not a whole number of 32-row tiles has no tile-aligned output shard, so it is
    // still refused; batch < 32 is the exception and is padded up to one tile in compute_output_specs
    // below. Batch above 32 is now supported when it is a whole number of batch tiles (B=64 is two):
    // the reader walks ceil(batch/32) tile rows of the output shard. The num_cores == batch check
    // further down is the real ceiling - one input core per user.
