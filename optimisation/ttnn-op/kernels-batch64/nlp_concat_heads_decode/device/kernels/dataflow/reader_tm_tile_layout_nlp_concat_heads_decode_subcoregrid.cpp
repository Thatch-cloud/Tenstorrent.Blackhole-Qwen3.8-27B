...
void kernel_main() {
    ...
    // Rows of the output shard per tile row: faces 0/1 then faces 2/3, face_h rows each. This is the
    // same rows-per-tile the program factory uses to place a head row within the input shard.
    constexpr uint32_t TILE_ROWS = 2 * face_h;

    for (uint32_t q = 0; q < batch; ++q) {
        // The output shard is (batch x head_dim) in tile layout: ceil(batch / TILE_ROWS) tile rows of
        // head_size_num_tiles tiles each, tiles row-major, so one whole tile row is head_size bytes.
        // User q lives at row (q % TILE_ROWS) of batch tile (q / TILE_ROWS); within a tile the first
        // face_h rows are face 0 and the rest are face 2 (the (row + face_h) form lands on it directly).
        // At batch <= TILE_ROWS the batch tile index is always 0 and this is byte-for-byte the original.
        uint32_t batch_tile = q / TILE_ROWS;
        uint32_t row_in_tile = q - batch_tile * TILE_ROWS;
        uint32_t wptr_offset = batch_tile * head_size + (row_in_tile < face_h
                                                             ? row_in_tile * SUBTILE_LINE_BYTES
                                                             : (row_in_tile + face_h) * SUBTILE_LINE_BYTES);
