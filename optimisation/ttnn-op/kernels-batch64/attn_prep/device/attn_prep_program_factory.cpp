...
// gate are pure copies the reader lands in its own ring, cbap::cpy); the writer stores the HDt
// full tiles at page b*HDt + t of the kind's output -- interleaved for q/gate, the KV update's
// height-sharded config for k/v (addressed through the tensor accessor) -- popping cbap::out for
// the computed kinds and cbap::cpy for the copied ones.
//
// ONE PRODUCER PER CIRCULAR BUFFER. A CB's producer-side page count lives in the producing RISC's
// own local interface and is *stored* into the shared L1 counter the consumer polls, not added to
// it. Two producers on different RISCs for one CB (here the reader on NCRISC for v/gate and the
// compute's PACK thread for q/k) therefore clobber each other: a push whose local count does not
// exceed what the other producer last stored is lost outright, and the consumer's cb_wait_front
// for the final block never satisfies. That is only survivable while no core is given both a
// computed and a copied instance -- true at B <= 32 (per_core is 1, or 2 with an even instance
// start, so a core's kinds are {q,k} or {v,gate}), false from B = 64 (per_core 3 mixes them).
// Hence the separate copy ring: cbap::out has exactly one producer, cbap::cpy exactly one.
...
constexpr uint32_t cpy = tt::CBIndex::c_23;   // [HDt] io   reader-copied block (v, gate) -> writer
}  // namespace cbap
...
tt::tt_metal::ProgramDescriptor AttnPrepProgramFactory::create_descriptor(
    ...
    add_cb(cbap::cpy, HDt, 2, df_io);  // reader -> writer only; never written by the compute
