...
// the compute; v/gate blocks go straight to cb_cpy, the writer's copy ring (no math). They must
// NOT go to cb_out: cb_out's only producer is the compute's PACK thread, and a second producer
// on another RISC clobbers the shared page count (see the program factory's header comment).
// q/k also get this row's cos and sin (page b*RDt + t, row 0 is the data). q_norm_w / k_norm_w
// row-0 tiles are read once.
//
...
constexpr uint32_t cb_src = 0, cb_blk = 1, cb_cpy = 23, cb_cos = 3, cb_sin = 4, cb_wq = 5, cb_wk = 6, cb_ones = 9;
...
void kernel_main() {
    ...
    for (uint32_t inst = inst_start; inst < inst_start + n_inst; ++inst) {
        ...
        const uint32_t target = math ? cb_blk : cb_cpy;
