...
//
// The block is popped from cb_out for the computed kinds (q, k) and from cb_cpy for the copied
// kinds (v, gate). Two rings, not one, because each has a single producer -- the compute's PACK
// thread and the reader respectively -- and a CB with two producers loses pushes (see the program
// factory's header comment). Both rings are bf16 with the same page size, so one tile size serves.
//
...
constexpr uint32_t cb_out = 2, cb_cpy = 23;
...
void kernel_main() {
    ...
    for (uint32_t inst = inst_start; inst < inst_start + n_inst; ++inst) {
        ...
        CircularBuffer cb(kind < 2 ? cb_out : cb_cpy);
        cb.wait_front(HDt);
