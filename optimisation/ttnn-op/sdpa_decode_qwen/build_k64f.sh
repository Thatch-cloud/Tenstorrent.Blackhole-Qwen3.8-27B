#!/usr/bin/env bash
# Build ~/opgraft-K64f on the rig: ~/opgraft-K64d plus the [QWEN-SDPA] stage-1+3 sdpa_decode
# (tail-only mask, forced compact tree scratch, per-call q_chunk_size sentinel, AND the K/V
# leader multicast across twin bundle entries - flag 0x2, spec section 7).
#
#   K64f = K64d's contents (attn_prep, nlp_concat_heads_decode, ...)
#        + _ttnncpp.so / _ttnn.so rebuilt in ttbuild over the F1-F12 factory
#        + sdpa_decode/  a copy of ttbuild's whole op directory, which is the image's
#                        directory plus the STAGE-3 reader_decode_qwen.cpp (R1-R5) and
#                        sdpa_flash_decode_qwen.cpp (C1-C2, the same file as K64e's).
...
#
# This is build_k64e.sh (which stays, to reproduce the served K64e) with the stage-3 factory
# (apply_factory_qwen.py --stage 3), the stage-3 kernel set (stage3/, make_qwen_kernels.py
# --stage 3) and two more strings checks: the new .so carries F9's '[QWEN-SDPA] KV-share twin
# bands' and NOT stage 1's 'KV share is not in this build'. A ttbuild left on the stage-1
# factory by a K64E_KEEP_STAGED run is recognised and restored from the saved 3e0a69af copy.
#
# lever_n_m3native_run_arm.sh mounts $KOPGRAFT64/sdpa_decode over the container's op
# directory only when the graft has one, so kernels JIT-compile from it, in a kernel cache
# keyed by the two kernels' bytes (so K64f never reuses K64e's reader binary).
#
# Usage (on the rig, after shipping this directory, stage3/ included, to $K64F_SRC):
#   bash ~/kwork64/k64f/build_k64f.sh
# Env: K64F_SRC (default ~/kwork64/k64f), K64F_BASE_GRAFT (~/opgraft-K64d), K64F_GRAFT
# (~/opgraft-K64f), K64F_IMAGE (the arm's default serving image), K64F_SKIP_IMAGE_COMPARE=1
# (only if that image is absent), K64F_KEEP_STAGED=1 (leave ttbuild on the qwen factory).
#
# ttbuild's sources are left as they were found (factory 3e0a69af, no qwen kernel files) unless
# K64F_KEEP_STAGED=1; everything it replaces is saved under $K64F_SRC/work-<stamp>/backup.
...
# rebuild, ...) would silently link the qwen factory. On a failure the EXIT trap restores
# and touches the sources but does not rebuild; it says so.
set -euo pipefail

S=${K64F_SRC:-$HOME/kwork64/k64f}
KS=$S/stage3   # the stage-3 kernel set (make_qwen_kernels.py --stage 3)
BASE=${K64F_BASE_GRAFT:-$HOME/opgraft-K64d}
GRAFT=${K64F_GRAFT:-$HOME/opgraft-K64f}
...
G=$GRAFT.partial
IMAGE=${K64F_IMAGE:-sha256:e41ef884f4c8e07ce6632511fac19364af35dc73d601f05f5191784a64a3a768}
...
FACTORY_QWEN=06167779a979ba1f35c78d1c002f9956215ca52a4ccd4531e0f5f5fe4e44191d
...
mkdir -p "$W/backup"
echo "### build_k64f $(date -Is) work=$W"
...
for path in "$KS/reader_decode_qwen.cpp" "$KS/sdpa_flash_decode_qwen.cpp" "$S/apply_factory_qwen.py" "$S/make_qwen_kernels.py"; do
  test -s "$path" || { echo "FAIL: $path missing" >&2; exit 1; }
  if grep -q $'\r' "$path"; then echo "FAIL: $path has CRLF line endings" >&2; exit 1; fi
done
need "shipped stage3/reader_decode_qwen.cpp" "$(hsha "$KS/reader_decode_qwen.cpp")" $READER_QWEN
need "shipped stage3/sdpa_flash_decode_qwen.cpp" "$(hsha "$KS/sdpa_flash_decode_qwen.cpp")" $COMPUTE_QWEN
...
python3 "$S/make_qwen_kernels.py" --stage 3 --reader "$W/reader_decode_all.cpp" --compute "$W/sdpa_flash_decode.cpp" --check "$KS"
...
restore_ttbuild() {
  if [ "$armed" = "0" ] || [ "$restored" = "1" ] || [ "${K64F_KEEP_STAGED:-}" = "1" ]; then
    return 0
  fi
...
on_exit() {
  ...
  if [ "$built" = "1" ] && [ "$rebuilt" = "0" ] && [ "${K64F_KEEP_STAGED:-}" != "1" ]; then
...
if [ "$cur" = "$FACTORY_UNPATCHED" ]; then
  ...
  echo "note  ttbuild factory already stage-3 qwen-patched (a K64F_KEEP_STAGED run); rebuilding from it"
  test -s "$S/sdpa_decode_program_factory.cpp.$FACTORY_BASE" || { echo "FAIL: no saved 3e0a69af factory to restore from" >&2; exit 1; }
  armed=1
elif [ "$cur" = "$FACTORY_BASE" ] || [ "$cur" = "$FACTORY_QWEN_STAGE1" ]; then
  if [ "$cur" = "$FACTORY_QWEN_STAGE1" ]; then
    # A K64E_KEEP_STAGED run left the stage-1 factory: patch from the saved 3e0a69af instead.
    echo "note  ttbuild factory is the stage-1 qwen factory (K64E_KEEP_STAGED); patching from the saved 3e0a69af"
    test -s "$S/sdpa_decode_program_factory.cpp.$FACTORY_BASE" \
      || cp -n "$HOME/kwork64/k64e/sdpa_decode_program_factory.cpp.$FACTORY_BASE" "$S/" 2>/dev/null \
      || { echo "FAIL: no saved 3e0a69af factory in $S or ~/kwork64/k64e to patch from" >&2; exit 1; }
  else
  ...
  python3 "$S/apply_factory_qwen.py" "$S/sdpa_decode_program_factory.cpp.$FACTORY_BASE" --stage 3 --out "$W/sdpa_decode_program_factory.cpp"
  need "patched factory (stage 3)" "$(hsha "$W/sdpa_decode_program_factory.cpp")" $FACTORY_QWEN
  armed=1
...
# ---------- 6. restore ttbuild ----------
if [ "${K64F_KEEP_STAGED:-}" = "1" ]; then
  echo "note  K64F_KEEP_STAGED=1: ttbuild keeps the qwen factory and kernels"
else
  restore_ttbuild
...
share=$(count "$so" '[QWEN-SDPA] KV-share twin bands')
stage1=$(count "$so" '[QWEN-SDPA] KV share is not in this build')
...
echo "strings: '[QWEN-SDPA] KV-share twin bands' $share  '[QWEN-SDPA] KV share is not in this build' $stage1"
test "$flags" -ge 1 || { echo "FAIL: the new _ttnncpp.so lacks the [QWEN-SDPA] factory branch" >&2; exit 1; }
test "$share" -ge 1 || { echo "FAIL: the new _ttnncpp.so lacks the stage-3 KV-share branch (F9)" >&2; exit 1; }
test "$stage1" -eq 0 || { echo "FAIL: the new _ttnncpp.so still refuses KV share (the stage-1 factory was linked)" >&2; exit 1; }
test "$combined" = "$(count "$BASE/_ttnncpp.so" qwen_draft_fp32_intermediates)" \
  || { echo "FAIL: the combined prefill factory's marker count moved from $BASE's" >&2; exit 1; }
...
need "graft reader_decode_qwen.cpp (stage 3)" "$(hsha $SD/kernels/dataflow/reader_decode_qwen.cpp)" $READER_QWEN
...
if [ "${K64F_SKIP_IMAGE_COMPARE:-}" = "1" ]; then
  echo "WARN  K64F_SKIP_IMAGE_COMPARE=1: the op directory was NOT compared against $IMAGE"
else
  docker image inspect "$IMAGE" >/dev/null || { echo "FAIL: image $IMAGE not present; set K64F_IMAGE or K64F_SKIP_IMAGE_COMPARE=1" >&2; exit 1; }
...
echo "K64F_TTNNCPP_SHA256=$(hsha "$G/_ttnncpp.so")"
echo "### build_k64f done $(date -Is)"
