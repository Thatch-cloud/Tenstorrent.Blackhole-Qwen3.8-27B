#!/usr/bin/env bash
# S0.K1 probe (the K1 head-sliced Q design, sections 3, 7 and 8): run probe_k1_card_b.py on the qualification
# card (QUAL_CARD, default card B), in the gate's image, with the SERVED decode SDPA graft (K64g) mounted
# exactly as lever_n_m3native_run_arm.sh mounts it for the gate's argv (KOPGRAFT64 and M3NATIVE_SDPA_PF=1):
# _ttnn.so, _ttnncpp.so (both paths), attn_prep, nlp_concat_heads_decode, sdpa_decode and sdpa. It times the
# served G8 batch-2 tail+share call, the G4 batch-2 tail+share call whose per-core work is the head-sliced
# G8 call's (K1a's per-call time), and the controls, at 2,304 / 33,024 / 66,048 / 131,328 keys, records
# every output's sha256, and prints one K1_PROBE go/no-go line. No build, no weights. NOT a CI workflow:
# invoke by hand on the rig.
#
#   bash run_probe_k1.sh               # the probe (about 45 min): checksums, eager and traced timing, verdict
#   WATCHER=1 bash run_probe_k1.sh     # THE FIRST HARDWARE PASS (the G4 B2 share shape was never qualified):
#                                      # TT_METAL_WATCHER=5 (NoC sanitiser, waypoints), checksums at 2,304 and
#                                      # 33,024 only, no timing, a per-call watchdog (WATCHDOG_S, default
#                                      # 120 s), a 900 s container timeout; the watcher's own log lands in
#                                      # $RESULTS/watcher-<stamp>/
#   PROBE_DRY_RUN=1 bash run_probe_k1.sh
#                                      # print the launch argv and exit: no node resolution, no holder check,
#                                      # no docker, nothing launched (the graft is checked when it exists)
#
# Before anything is launched the graft must be the served one: $G/_ttnncpp.so is EXPECT_TTNNCPP_SHA256
# (default K64g's 134bc834..., the v192 gate's QWEN_FAST_RUNTIME_BINARY_SHA256; set it empty to skip), the
# two qwen kernels under $G/sdpa_decode are the served 280a847f / 8776fcc7, and $G/MANIFEST.sha256 verifies
# (the arm requires that under M3NATIVE_SDPA_PF=1). The harness checks the mapped binary and the mounted
# kernels again inside the container, and the container prints their sha256s first.
#
# Env: KOPGRAFT64 (~/opgraft-K64g), IMAGE (default image P3, the v194-v199 gate image), RESULTS
# (~/kwork64/k1probe/<card tag>: card-b, card-m, card-a), CARD_B_ARGS (extra harness args, appended last,
# e.g. "--capacities 33024,131328 --rounds 1"), EXPECT_TTNNCPP_SHA256, WATCHER=1, WATCHDOG_S,
# PROBE_DRY_RUN=1, QUAL_CARD (the target board id under /dev/tenstorrent/by-id; default card B,
# blackhole-F36F768B9A5CAFA0; card M or card A, the serving pair, is refused unless ALLOW_SERVING_CARD=1,
# which prints a loud warning). QWEN_SDPA_TREE_SCRATCH_ROUNDS=1 is set as the arm sets it (the G8 legacy
# calls do not fit L1 without it); every run gets a fresh kernel cache.
#
# ON A HANG (the WATCHDOG line and exit 3; exit 1 with 'Timeout (' in the log, the faulthandler backstop
# when a blocking call holds the GIL; or the timeout's 124 / 137):
...
role=${1:-probe}
G=${KOPGRAFT64:-$HOME/opgraft-K64g}
...
# <<< qual_card.sh
qual_card_select
R=${RESULTS:-$HOME/kwork64/k1probe/$QUAL_TAG}
OPS=/opt/tt-metal/ttnn/cpp/ttnn/operations
DRY=${PROBE_DRY_RUN:-0}
name=qwen-k1probe-$QUAL_TAG
stamp=$(date +%Y%m%dT%H%M%S)
timeout_s=3600   # 4 capacities x 5 shapes: cold JIT, 2 x 20 eager calls and a replayed 16-call trace each

case "$role" in
  probe) ;;
  *) echo "unknown role $role (run_probe_k1.sh has one role: probe)" >&2; exit 1 ;;
esac
...
for file in probe_k1_card_b.py test_sdpa_decode_qwen_card_m.py; do
  test -s "$here/$file" || { echo "refusing: $here/$file missing" >&2; exit 1; }
done
...
if [ "$DRY" = 1 ] && [ ! -e "$G" ]; then
  ...
  for part in _ttnn.so _ttnncpp.so attn_prep nlp_concat_heads_decode sdpa_decode sdpa MANIFEST.sha256; do
    test -e "$G/$part" || { echo "refusing: $G/$part missing (the probe needs the served K64g graft, build_k64g.sh)" >&2; exit 1; }
  done
  (cd "$G" && sha256sum -c --quiet MANIFEST.sha256 >&2) \
    || { echo "refusing: $G/MANIFEST.sha256 does not verify (not the graft build_k64g.sh made)" >&2; exit 1; }
  so_sha=$(sha256sum "$G/_ttnncpp.so" | cut -c1-64)
  if [ -n "$EXPECT" ] && [ "$so_sha" != "$EXPECT" ]; then
    echo "refusing: $G/_ttnncpp.so is ${so_sha:0:16}, not ${EXPECT:0:16} (K64g as served; EXPECT_TTNNCPP_SHA256= skips)" >&2
    exit 1
  fi
  ...
  echo "### graft $G: _ttnncpp.so ${so_sha:0:16}, qwen kernels ${READER_QWEN:0:8} / ${COMPUTE_QWEN:0:8}, manifest verified"
fi
...
args=(--out "/results/$role-$stamp.json" --expect-binary-sha256 "$EXPECT")
...
WM=()
if [ "${WATCHER:-}" = "1" ]; then
  # One pass under the NoC sanitiser over every shape's poisoned and legacy call at the early-return and
  # a multi-chunk capacity; timing is meaningless here. CARD_B_ARGS, appended last, can widen it.
  timeout_s=900
  args+=(--capacities 2304,33024 --no-timing --watchdog "${WATCHDOG_S:-120}")
...
argv=(docker run --rm --name "$name" --network none
  ...
  --mount "type=bind,src=$here/probe_k1_card_b.py,dst=/bench/probe_k1_card_b.py,readonly"
  --mount "type=bind,src=$here/test_sdpa_decode_qwen_card_m.py,dst=/bench/test_sdpa_decode_qwen_card_m.py,readonly"
  ...
  "sha256sum /opt/tt-metal/build_Release/lib/_ttnncpp.so /opt/tt-metal/build_Release/ttnn/_ttnncpp.so $OPS/transformer/sdpa_decode/device/kernels/dataflow/reader_decode_qwen.cpp $OPS/transformer/sdpa_decode/device/kernels/compute/sdpa_flash_decode_qwen.cpp 2>&1; exec python3 -B /bench/probe_k1_card_b.py \"\$@\""
  "$role" "${args[@]}" ${extra[@]+"${extra[@]}"})
echo "### k1probe $role $stamp card=$QUAL_CARD ($QUAL_TAG) node=$node image=${IMAGE:7:12} graft=$G watcher=${WATCHER:-0}"
...
# Anchored: the harness's summary line 'SDPA_K1_PROBE passed=...' comes after the verdict and also
# contains 'K1_PROBE '.
echo "### $(grep -E '^K1_PROBE ' "$log" | tail -1 || echo 'no K1_PROBE line')"
hung=0
case "$status" in
  3|124|137) hung=1 ;;
