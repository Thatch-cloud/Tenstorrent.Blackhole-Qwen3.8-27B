#!/usr/bin/env bash
# Run test_sdpa_decode_qwen_card_m.py on the qualification card (QUAL_CARD, default card B), in the
# serving image.
#
...
# image), RESULTS (~/kwork64/k64f/<card tag>: card-b, card-m, card-a - a candidate reads only
# references taken on its own board), CARD_M_ARGS (extra args, appended last, e.g.
...
#
# Every run gets a fresh kernel cache. ON A HANG (the WATCHDOG line, exit 3, or the timeout's 124):
...
echo "### $mode $stamp card=$QUAL_CARD ($QUAL_TAG) node=$node image=${IMAGE:7:12} graft=$G"
