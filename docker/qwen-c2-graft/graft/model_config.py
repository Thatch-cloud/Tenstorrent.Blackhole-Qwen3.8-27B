...
_QWEN_VERIFY_T1_KEPT = ('in0_block_w', 'per_core_M', 'fuse_batch', 'mcast_in0')


def _qwen_verify_t1_configs(args, mesh_device, tpc, ttnn, m, tp):
    """Verify-trace T1 #11 (QWEN_FAST_VERIFY_T1=1; lever_n_m3native_patch section A2): the
    ...
    if -(-88 // args.decode_grid_w) > grid.y:
        return "worker grid %dx%d cannot hold %d cores %d wide" % (grid.x, grid.y, 88, args.decode_grid_w)
    before = (args.attn_qkv_decode_1d_progcfg_64, args.mlp_w1_decode_1d_progcfg_64)
    after = (
        tpc.create_matmul_1d_decode_progcfg(
            m, args.dim, args.attn_qkv_fused_dim_tp, num_cores=44, grid_w=args.decode_grid_w
        ),
        tpc.create_matmul_1d_decode_progcfg(
            m,
            args.dim,
...
class Qwen36ModelArgs(ModelArgs):
    ...
    def _init_tp_config(self, mesh_device):
        ...
        if os.environ.get("QWEN_FAST_VERIFY_T1") == "1" and "matmul_configs" not in [
            _qwen_name.strip() for _qwen_name in os.environ.get("QWEN_FAST_VERIFY_T1_SKIP", "").split(",")
        ]:
            ...
            if _qwen_t1_skipped is None:
                _qwen_logger.info("[PINDIAG] verify t1 engaged site=matmul_configs attn_qkv_64_cores=44 mlp_w1_64_cores=88 grid_w={}",
                                  self.decode_grid_w)
            else:
                _qwen_logger.warning("[PINDIAG] verify t1 matmul configs not applied: {}", _qwen_t1_skipped)
