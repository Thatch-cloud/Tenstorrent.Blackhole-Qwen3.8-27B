...
class TPAttention:
    ...
    def __init__(self, mesh, args, tw, tt_ccl):
        ...
        # Prefill lever #1 (QWEN_FAST_SDPA_PF=1, section I): the chain word, or None (the served config).
    ...
    def forward_prefill_paged(
        ...
        if getattr(self, "_sdpa_pf_word", None) is not None:
            # Prefill lever #1 (QWEN_FAST_SDPA_PF=1): the same config plus the G6 K/V chain word on the
...
# Prefill lever #1 (QWEN_FAST_SDPA_PF=1; lever_n_m3native_patch section I): the per-call opt-in to the
# G6 K/V chain of the chunked prefill SDPA (optimisation/ttnn-op/sdpa_prefill_chain). Inert unless the flag is 1.
_QWEN_PF_TAG = 0x5EFA0000
_QWEN_PF_FLAGS = (0x1, 0x3, 0x5, 0x7)  # the production flag sets, each swept by card-M Q1
_QWEN_PF_DEFAULT_FLAGS = 0x3
_QWEN_PF_MARKER = b'[QWEN-SDPA-PF] flags='
_QWEN_PF_ROWS = (512, 1024, 2048)  # the card-M-qualified S
_QWEN_PF_CHUNK = 128
_QWEN_PF_STATE = {}
...
def _qwen_pf_flags(environ):
    """QWEN_FAST_SDPA_PF_FLAGS (unset or empty: the default), refused unless a production flag set."""
    text = (environ.get("QWEN_FAST_SDPA_PF_FLAGS") or "").strip() or "0x%X" % _QWEN_PF_DEFAULT_FLAGS
    try:
        flags = int(text, 0)
    except ValueError:
        flags = None
    if flags not in _QWEN_PF_FLAGS:
        raise RuntimeError("QWEN_FAST_SDPA_PF_FLAGS must be a production flag set (%s), got %r"
                           % (", ".join("%#x" % value for value in _QWEN_PF_FLAGS), text))
    return flags


def _qwen_pf_word(environ=None, binary_check=None):
    """None (the served config) unless QWEN_FAST_SDPA_PF=1, else 0x5EFA0000 | flags. Bad flags and a
    ...
    if process:
        environ = os.environ
    if environ.get("QWEN_FAST_SDPA_PF") != "1":
    ...
    if not (_qwen_pf_binary_has_marker if binary_check is None else binary_check)():
        raise RuntimeError("QWEN_FAST_SDPA_PF=1 but the loaded _ttnncpp.so lacks the [QWEN-SDPA-PF] chain "
                           "factory (mount a K64g-or-later graft: KOPGRAFT64)")
    word = _QWEN_PF_TAG | flags
    rows = "/".join(str(value) for value in _QWEN_PF_ROWS)
    _qwen_pf_log("[PINDIAG] sdpa prefill kvchain flags=%#x rows=%s chunk=%d" % (word, rows, _QWEN_PF_CHUNK))
    if process:
