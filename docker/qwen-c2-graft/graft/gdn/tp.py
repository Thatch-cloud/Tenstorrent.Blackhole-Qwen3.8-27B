...
from models.experimental.gated_attention_gated_deltanet.tt.ttnn_delta_rule_ops import (
    ...
    recurrent_gated_delta_rule_decode_packed_ttnn,
)
...
# Lever #2 (QWEN_FAST_GDN_PREFILL_CONV=1): gdn_prefill_conv_exact replaces the valid_len FIR + q/k/v slices.
...
def _qwen_prefill_conv_on(layer, qkv, cstate, T, valid_len):
    if os.environ.get("QWEN_FAST_GDN_PREFILL_CONV") != "1":
    ...
    from models.demos.blackhole.qwen36.tt.gdn import gdn_prefill_conv_exact as _pcx
    ...
    if reason is not None:
        ...
        _qwen_logger.warning("[PINDIAG] GDN prefill conv fell back to the FIR: {} (T={} valid_len={})", reason, T, valid_len)
...
def _qwen_prefill_conv(layer, qkv, cstate, valid_len, T):
    ...
    if id(layer) == state["first"]:
        ...
        state["chunk"] += 1
        _qwen_logger.info("[PINDIAG] GDN prefill conv engaged: chunk {} previous_chunk_calls={} calls={} T={} valid_len={} carry={}",
    ...
    if state["expected"] is not None and state["chunk_calls"] == state["expected"]:
        ...
        _qwen_logger.info("[PINDIAG] GDN prefill conv chunk complete: chunk {} calls={}", state["chunk"], state["chunk_calls"])
    if state["audited"] < int(os.environ.get("QWEN_FAST_GDN_PREFILL_CONV_AUDIT", "0") or 0):
        ...
        _qwen_logger.info("[PINDIAG] GDN prefill conv audit {} exact={} T={} valid_len={} carry={} mismatches={}",
...
class TPGatedDeltaNet:
    ...
    def forward_prefill(self, x, chunk_size=128, valid_len=None, capture_state=False, return_state=False):
        ...
        if self._gdn_conv1d and valid_len is None:
            ...
            # Lever #2 (QWEN_FAST_GDN_PREFILL_CONV=1): one tiled launch replaces the FIR + q/k/v slices.
    ...
    def remap_slots(self, remap):
        ...
        # vLLM sizes slot_remap to the LIVE batch, while self.B is the full slot
    ...
    def forward_decode(self, x):
        ...
        # Milestone 4 (QWEN_GDN_PROJ_DIRECT=1): keep the fused [qkv|z|a|b] projection output whole
        ...
        # K's first kernel (QWEN_GDN_CONV_GATES=1): conv shift-register + FIR + SiLU + both
        ...
        if self._conv_gates_enabled():
            ...
            if not (_direct and _ng_fold):
                ttnn.deallocate(qkv)  # in direct+fold mode qkvzab is still z: freed after the recurrence
            if _direct:
        ...
        # Milestone 2 (QWEN_GDN_PACKED_QKV=1, needs the conv+gates kernel engaged): the recurrence
        ...
        # QWEN_GDN_FUSED_INPLACE=1: ask the fused op to write the new state straight into
        # rec_state (its _inplace variant) instead of allocating h and copying it back.
        # Legal only when init_state IS the persistent buffer (B == Bmax, no slice). The
        # composed path ignores the flag, so the caller decides by IDENTITY below, not env.
        _want_inplace = (
            os.environ.get("QWEN_GDN_FUSED_INPLACE") == "1"
            and self._stable_state and B == Bmax and init_state is self.rec_state
        )
        # Milestone 3 (QWEN_GDN_NORM_GATE=1, needs the packed path): the norm+gate kernel takes o
        ...
        # The op returns *in.initial_state, but nanobind hands back a NEW Python wrapper
        # around the same device buffer, so `is` sees two objects. Compare the buffer.
        try:
            _did_inplace = _want_inplace and new_rec.buffer_address() == init_state.buffer_address()
        except Exception:
            _did_inplace = False
        if _want_inplace and not getattr(self, "_inplace_logged", False):
            self._inplace_logged = True
            print(f"QWEN_GDN_FUSED_INPLACE requested; in-place happened={_did_inplace}"
                  f" (same buffer={new_rec.buffer_address() == init_state.buffer_address()})", flush=True)
        ...
        if self._stable_state:
            ...
            if B == Bmax:
                if _did_inplace:
                    pass  # new_rec IS rec_state: nothing to copy, and freeing it would free the state
                else:
        ...
        if _ng:
            ...
            ttnn.deallocate(z)  # in direct+fold mode this is qkvzab itself
