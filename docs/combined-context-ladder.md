...
Both attempts of `35172695250` stop before model loading: host full I/O stall is 4.60% and 55.38%, above the unchanged 1% admission limit. They do not exercise the audit-memory fix. The next 131K-only tag, `experiment/combined-ladder-wide-cache-v4`, allows up to four 15-second admission samples separated by 15-second waits (105 seconds total). Every rejected sample is retained; a fully busy host still fails without loading weights. This does not impose a benchmark timeout or establish host isolation after admission.
