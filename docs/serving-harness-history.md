# Historical serving and terminal harness guide

Imported from PR #1 (208d612), preserving its August baseline and proposed harness configuration. **Not the current T16 combined runtime.** The 64K BF8 KV change, streaming tool-call parsing, installed harness configs and repeated accuracy/readiness checks are not qualified by the current ladder. Do not deploy this as the measured fast recipe. The old P300 physical descriptor below is historical; use the explicit P150 pair descriptor for the current four-link setup. See [current reproduction](runtime-reproduction.md).
...
**Every flag is load-bearing.** See [docs/gotchas.md](gotchas.md) for what
...
| `QWEN_SDPA_BF8=1` | at 64k × B=8 the paged-KV allocator OOMs — required at this `max_model_len`, with an accuracy caveat ([gotchas](gotchas.md)) |
