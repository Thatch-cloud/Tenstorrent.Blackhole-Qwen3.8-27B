...
from gdn_qk_double_buffer_gate import KERNEL, validate_report


def fixture():
    report = dict(passed=True, closed_cleanly=True, norm_unchanged=True,
        state_math_unchanged=True, shared_qk_preparation=True, qk_double_buffer=True,
...
class DoubleBufferGateTests(unittest.TestCase):
    ...
    def test_missing_changed_or_promoted_evidence_rejected(self):
        for mutate in (lambda report: report['checks'].pop(),
                ...
                lambda report: report['generated_kernels'][0].update(extra_cb_bytes=2048),
