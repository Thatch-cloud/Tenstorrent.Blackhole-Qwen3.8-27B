...
import gdn_direct_window_experiment as experiment
from dspark_request_experiment import summarize
from test_gdn_direct_window_comparison import fixture
...
class DownGridExperimentTests(unittest.TestCase):
    def test_all_six_requests_keep_norm_prefetch_and_restore_routes(self):
        with TemporaryDirectory() as temporary:
            ...
            environment = {name: '1' for name in ('QWEN_GDN_DIRECT_WINDOW', 'QWEN_FROZEN_COMBINED_RUNTIME',
            ...
            with patch.dict('sys.modules', modules), patch.dict('os.environ', environment, clear=True), \
                    patch.object(experiment, '__file__', str(directory / 'gdn_direct_window_experiment.py')), \
                    patch.object(experiment, 'qualify', return_value={}), patch.object(experiment, 'scoped_direct_windows', scope):
            ...
            self.assertEqual(report['gdn_direct_window_comparison']['arms']['direct']['committed_tg'], 120)
            self.assertEqual(report['gdn_direct_window_sources'], report['gdn_direct_window_sources_after'])
