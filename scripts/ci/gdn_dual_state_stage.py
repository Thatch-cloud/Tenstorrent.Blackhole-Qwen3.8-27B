"""Stage synthetic state-copy fan-out with complete prefix-state replay checks."""
...
def adapt(source):
    source = replace_once(source, 'from gdn_shared_qk_pipeline import build as build_pipeline',
        'from gdn_dual_state_copy import build as build_pipeline, BUILD_RECORDS')
    source = replace_once(source,
        'rows=16, norm_unchanged=True, state_math_unchanged=True, shared_qk_preparation=True)',
        'rows=16, norm_unchanged=True, state_math_unchanged=True, shared_qk_preparation=True,\n'
        '        dual_state_copy=True, generated_kernels=BUILD_RECORDS)')
...
def main():
    ...
    helper = Path(__file__).with_name('gdn_dual_state_copy.py').read_bytes()
    (scripts / 'gdn_dual_state_copy.py').write_bytes(helper)
