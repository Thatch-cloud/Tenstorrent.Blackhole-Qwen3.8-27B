...
import gdn_qk_double_buffer_experiment as experiment
from dspark_request_experiment import summarize
from test_gdn_qk_double_buffer_comparison import fixture


class DoubleBufferExperimentTests(unittest.TestCase):
    def test_all_six_requests_keep_norm_prefetch_and_restore_routes(self):
        with TemporaryDirectory() as temporary:
            ...
            @contextmanager
            def scope(evidence):
                audit = dict(kernels=[experiment.KERNEL] * 96, restored=False)
                active.append(True)
                try:
                    yield audit
                finally:
            ...
            environment = {name: '1' for name in ('QWEN_GDN_QK_DOUBLE_BUFFER', 'QWEN_FROZEN_COMBINED_RUNTIME',
            ...
            with patch.dict('sys.modules', modules), patch.dict('os.environ', environment, clear=True), \
                    patch.object(experiment, '__file__', str(directory / 'gdn_qk_double_buffer_experiment.py')), \
                    patch.object(experiment, 'qualify', return_value={}), patch.object(experiment, 'scoped_double_buffer', scope):
            ...
            self.assertEqual(report['qk_double_buffer_comparison']['arms']['double_buffered_qk']['committed_tg'], 120)
            self.assertEqual(report['qk_double_buffer_sources'], report['qk_double_buffer_sources_after'])
