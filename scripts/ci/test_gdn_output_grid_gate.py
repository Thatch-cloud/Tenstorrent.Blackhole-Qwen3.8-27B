...
from gdn_output_grid_gate import LOCAL_SOURCES, NATIVE_SOURCE, qualify
...
class OutputGateTests(unittest.TestCase):
    def test_exact_matrix_and_source_mutation(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            report = dict(passed=True, closed_cleanly=True, native_configuration_unchanged=True, grids=dict(control=[11, 3], candidate=[11, 6]), backend='simulator',
            ...
            with patch('gdn_output_grid_gate.REPORT_SHA256', hashlib.sha256(report_path.read_bytes()).hexdigest()):
