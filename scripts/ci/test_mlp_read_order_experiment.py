...
import mlp_read_order_experiment as experiment
from mlp_read_order_comparison import SCHEDULE
from mlp_read_order_gate import REPORT_SHA256
from mlp_read_order_stage import adapt
from dspark_request_experiment import summarize
from test_mlp_read_order_comparison import fixture
...
class ReadOrderExperimentTests(unittest.TestCase):
    def test_matched_runtime_routes_restore_between_all_six_requests(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for name in ('mlp_read_order_gate.py', 'mlp_read_order_comparison.py',
                    'mlp_read_order_experiment.py', 'mlp_weight_read_order.py'):
            ...
            def original_route(value, arm):
                enabled = value['weight_read_order']['staggered']
            ...
            environment = {name: '1' for name in ('QWEN_MLP_READ_ORDER', 'QWEN_FROZEN_COMBINED_RUNTIME',
                'QWEN_CARDS_ALLOCATED', 'QWEN_HARDWARE_TESTS')}
            with patch.dict('sys.modules', modules), patch.dict('os.environ', environment, clear=True), \
                    patch.object(experiment, '__file__', str(directory / 'mlp_read_order_experiment.py')), \
            ...
            self.assertEqual(report['read_order_comparison']['arms']['staggered']['committed_tg'], 120)
            self.assertEqual(report['read_order_sources'], report['read_order_sources_after'])
    ...
    def test_stage_schedule_keeps_winning_publication_policy_in_every_arm(self):
        ...
        self.assertIn('QWEN_MLP_READ_ORDER', result['run-dspark-hardware.sh'])
