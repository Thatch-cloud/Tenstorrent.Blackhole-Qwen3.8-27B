"""Admit the exact full-recurrence Q/K double-buffer simulator result and generated kernel."""

import hashlib
import json
from pathlib import Path

from gdn_qk_double_buffer import reader as transform
from gdn_shared_qk_gate import LOCAL_SOURCES, API_SOURCES
from gdn_multitoken import HASHES, KERNEL_ROOT


REPORT_SHA256 = '181173b7b6ab22e36f2068cce66255c621d5d30915ab7659ce9a5b6c755ea21c'
CONTROL_SHA256 = '3a670f7e24dd53b8edcbaaac2934a9fc95cee1b46a9af8ccb5d0e90ad2f4df22'
CANDIDATE_SHA256 = '3adef6c2b0a46d5ee6ba5172ab279e00b0f37d4a8ffa6aae1b293e82f6c3d889'
...
def validate_report(report):
    if (any(report.get(key) is not True for key in ('passed', 'closed_cleanly', 'norm_unchanged',
        ...
        raise ValueError('Complete unchanged-source Q/K double-buffer recurrence evidence required')
...
def qualify(evidence, directory, runtime):
    ...
    if hashlib.sha256(raw).hexdigest() != REPORT_SHA256:
        raise ValueError('Exact reviewed Q/K double-buffer simulator artifact required')
    ...
    for name, path in paths.items():
        if hashlib.sha256(path.read_bytes()).hexdigest() != report['sources'].get(name):
            raise ValueError('Double-buffer dependency differs from simulation: ' + name)
