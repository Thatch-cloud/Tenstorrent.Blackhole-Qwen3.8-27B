...
import gdn_wait_clock_combined as combined
from gdn_wait_clock import instrument
from gdn_wait_clock_gate import REPORT_SHA256
from test_gdn_wait_clock import WaitClockTests
from test_mlp_compute_clock_combined import Capture, Engine


class CombinedRecurrenceTests(unittest.TestCase):
    def setUp(self):
        self.source = WaitClockTests().source()
        ...
        self.bank = combined.CombinedWaitCapture(self.operations, self.model, self.captures,
    ...
    def test_partial_bindings_alias_and_unqualified_admission_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, 'alias'):
            combined.CombinedWaitCapture(self.operations, self.model, self.captures, self.candidate,
    ...
    def test_staging_remains_a_single_audited_combined_request(self):
        from gdn_wait_clock_combined_stage import adapt
        ...
        self.assertIn('QWEN_GDN_WAIT_CLOCK_COMBINED', result['run-dspark-hardware.sh'])
        with self.assertRaises(ValueError):
            adapt(result)
        workflow = (Path(__file__).parents[2] / '.github/workflows/qwen-gdn-wait-clock-combined.yml').read_text()
        self.assertIn('35249071221', workflow)
