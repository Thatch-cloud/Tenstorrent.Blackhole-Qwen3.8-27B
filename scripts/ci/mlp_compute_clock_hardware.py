...
SIMULATOR_SHA256 = '027af7feffa73ee1d3fca5c47a265d810e1a59ab27bac9ad07c9faad401f7cb3'
...
def verify_sources(directory):
    ...
    manifest = json.loads((directory / 'compute-clock-hardware-sources.json').read_text())
...
def stage(directory, evidence):
    ...
    retained(evidence)
    manifest = json.loads((evidence / 'compute-clock-candidate.json').read_text())
    ...
    runner_source = replace_once(runner.read_text(), '/experiment-scripts/ci/wait-zone-hardware-suite.sh',
        '/experiment-scripts/ci/mlp-compute-clock-hardware-suite.sh')
    (directory / 'run-mlp-compute-clock-hardware.sh').write_bytes(runner_source.encode())
    names = {*manifest['sources'], 'mlp_compute_clock_hardware.py', 'mlp_compute_clock_report.py', 'mlp_clock_report.py',
        'run-mlp-compute-clock-hardware.sh', 'mlp-compute-clock-hardware-suite.sh'}
    hashes = {name: hashlib.sha256((directory / name).read_bytes()).hexdigest() for name in sorted(names)}
    (directory / 'compute-clock-hardware-sources.json').write_text(json.dumps(hashes, indent=2) + '\n')
...
def main():
    ...
    if options.command == 'stage':
        ...
        (options.output / 'compute-clock-hardware.json').write_text(json.dumps(summary, indent=2) + '\n')
