"""Matched complete requests; only the T16 weight-reader DMA scheduling changes."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('weight_weight_pipeline', {}).get('pipelined'), value.get('instrumented_timing'))
            for value in requests] != list(SCHEDULE):
        raise ValueError('Two fresh audits followed by complete unchanged/pipelined/pipelined/unchanged requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['weight_weight_pipeline']['pipelined'] is enabled]) for name, enabled in (('unchanged', False), ('pipelined', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(unchanged_tg=baseline, pipelined_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['pipelined']['committed_tg'] / arms['unchanged']['committed_tg'] - 1),
