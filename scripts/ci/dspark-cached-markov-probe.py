"""Exact 15-step device-cached Markov feedback; no full-vocabulary speed qualification."""
...
from dspark_cached_markov import BiasCache, execute as cached_execute
from markov_sparse_build import validate
from feature_projection import require_projection_environment
from gdn_multitoken_conv import addresses, release_owned


SOURCES = ('dspark-cached-markov-probe.py', 'dspark_markov_device.py',
    'dspark_cached_markov.py', 'markov_cache_program.py', 'markov_cache_control.cpp',
    'markov_cache_control.hpp', 'markov_cache_pipeline.py', 'markov_cache_mask.cpp', 'markov_cache_payload.cpp', 'dspark_score_layout.py', 'dspark_score_layout_io.cpp',
...
def main():
    ...
    parser.add_argument('--vocabulary', type=int, choices=(64,), default=64)
    options = parser.parse_args()
    require_projection_environment(os.environ, False)
    if options.output.exists():
        raise ValueError('Fresh Markov chain evidence required')
    root = Path(__file__).parent
    qualification = validate(os.environ['TT_METAL_HOME'], '/experiment/results/markov-sparse-build.json')
    import torch
    import ttnn
    ...
    report = dict(passed=False, closed_cleanly=False, backend='simulator', scope=__doc__,
        vocabulary=options.vocabulary, proposals=15, sources=hashes(), build=qualification, full_vocabulary_qualified=False, performance_qualified=False, cache_reset_epochs=[],
        eager_checks=[], replay_checks=[], input_checks=[], weight_checks=[])
    mesh, trace, cache, persistent, temporary = None, None, None, [], []
