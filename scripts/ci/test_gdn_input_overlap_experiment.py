...
import gdn_input_overlap_experiment as experiment
from dspark_request_experiment import summarize
from test_gdn_input_overlap_comparison import fixture


class InputOverlapExperimentTests(unittest.TestCase):
    def test_all_six_requests_keep_norm_prefetch_and_restore_routes(self):
        with TemporaryDirectory() as temporary:
            ...
            environment = {name: '1' for name in ('QWEN_GDN_INPUT_OVERLAP', 'QWEN_FROZEN_COMBINED_RUNTIME',
            ...
            with patch.dict('sys.modules', modules), patch.dict('os.environ', environment, clear=True), \
                    patch.object(experiment, '__file__', str(directory / 'gdn_input_overlap_experiment.py')), \
                    patch.object(experiment, 'qualify', return_value={}), patch.object(experiment, 'scoped_overlap', scope):
            ...
            self.assertEqual(report['input_overlap_comparison']['arms']['overlapped_inputs']['committed_tg'], 120)
            self.assertEqual(report['input_overlap_sources'], report['input_overlap_sources_after'])
