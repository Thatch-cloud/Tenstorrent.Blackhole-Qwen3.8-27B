...
from gdn_wait_clock_capture import WaitClockCapture
from gdn_wait_clock_combined import CombinedWaitCapture
from gdn_wait_clock_gate import HELPERS, qualify


FILES = (*HELPERS, 'gdn_wait_clock_gate.py', 'gdn_wait_clock_report.py',
    'gdn_wait_clock_combined.py', 'gdn_wait_clock_experiment.py')
...
def run_loaded_requests(operations, generator, model, collectives, tokenizer, pages, kv_cache, parameters,
    ...
    import gdn_wait_clock_pipeline
    from verifier_engine import VerifierEngine
    from gdn_multitoken_conv import release_owned

    if (any(os.environ.get(name) != '1' for name in ('QWEN_GDN_WAIT_CLOCK_COMBINED',
    ...
    admission = qualify(directory / 'wait-clock-evidence', directory, os.environ['TT_METAL_HOME'])
    ...
    def finish(result, summarize):
        ...
        result.update(pp=None, committed_tg=None, fresh_context_audit=True, ctx_tokens=4096,
            ...
            gdn_wait_clock_combined=bank.summary())
        finished.append(True)

    try:
        progress('allocate_wait_clock_pages_before_request_warmup')
        captures = [WaitClockCapture(operations, model.mesh_device, owned) for unused in range(48)]
        bank = CombinedWaitCapture(operations, model, captures, gdn_wait_clock_pipeline, admission)
        ...
        def measure(*args, **kwargs):
            ...
            result['gdn_wait_clock_combined'] = bank.summary()
            report['gdn_wait_clock_combined'] = result['gdn_wait_clock_combined']
            return result
        ...
        if bank is not None:
            try:
                ...
                model._qwen_wait_clock_failed_owned = owned
                raise
        release_owned(operations, owned)
        report['gdn_wait_clock_sources'] = sources
        report['gdn_wait_clock_sources_after'] = fingerprints()
        if report['gdn_wait_clock_sources_after'] != sources:
