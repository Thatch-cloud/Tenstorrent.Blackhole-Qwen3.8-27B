"""Simulator comparison of native T16 GDN partial outputs in DRAM versus L1."""
...
def main():
    ...
    paths = [Path(__file__), Path(__file__).with_name('gdn_output_l1_scope.py'),
        Path(__file__).with_name('model_batch.py'), Path(__file__).with_name('attention_batch.py'),
    ...
    try:
        ...
        layers = [SimpleNamespace(is_full_attention=False, attention=SimpleNamespace(
            args=SimpleNamespace(proj_1d_decode=True, gdn_out_decode_1d_progcfg=program),
            cfg=native.COMPUTE_HIFI2, tw={'out': weight}, _row_proj=control)) for index in range(48)]
        arm = GDNOutputL1Arm(ttnn, SimpleNamespace(layers=layers), native.matmul_1d_decode)
        def compare(expected, actual, label):
            ...
            for chip, (reference, candidate) in enumerate(zip(expected_shards, actual_shards, strict=True)):
                ...
                if not torch.isfinite(reference).all() or not torch.equal(reference, candidate):
                    raise AssertionError('L1 output changed the native projection')
        ...
        with arm.install():
            ...
            for index, value in enumerate(inputs):
                actual = retain(layers[0].attention._row_proj(value, weight))
                if actual.memory_config() != ttnn.L1_MEMORY_CONFIG:
                    raise AssertionError('Candidate did not use L1')
                compare(references[index], actual, 'eager_' + str(index))
            save('capture')
            trace, output = capture_operation(ttnn, mesh,
                lambda: layers[0].attention._row_proj(inputs[0], weight))
            traces.append(trace)
            retain(output)
            for index in (1, 2, 0):
        ...
        report['scope_restored'] = not arm.active
