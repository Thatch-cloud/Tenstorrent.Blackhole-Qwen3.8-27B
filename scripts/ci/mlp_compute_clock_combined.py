...
class CombinedComputeClockCapture:
        ...
        for capture in self.captures:
            ...
            for chip, address in enumerate(addresses):
                ...
                if identity in identities:
                    raise ValueError('Sample storage aliases another layer')
    ...
    def projection(self, mesh, weights, **kwargs):
        ...
        if (not self.active or self.failed or index >= 64 or mesh is not self.model.mesh_device
                ...
                or 'compute_samples' in kwargs):
            raise ValueError('Ordered native-weight T16 projections required')
        projection = self.candidate(mesh, weights, compute_samples=self.captures[index].buffer, **kwargs)
    ...
    def summary(self):
        ...
        return dict(diagnostic_only=True, committed_tg=None, layers=64,
            sampled_verifier_replays=2, records=list(self.records), compute_samples_releasable=True)
