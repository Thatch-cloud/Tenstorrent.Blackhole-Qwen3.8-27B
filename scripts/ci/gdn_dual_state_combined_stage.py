"""Stage complete matched dual-state requests on the frozen winning T16 recipe."""
...
from gdn_dual_state_gate import validate_report, REPORT_SHA256, DEPENDENCIES
...
def adapt(sources):
    ...
    result['dspark-target-hardware.py'] = replace_once(result['dspark-target-hardware.py'],
        ...
        '            from gdn_dual_state_experiment import run_loaded_requests')
    result['run-dspark-hardware.sh'] = replace_once(result['run-dspark-hardware.sh'],
        '    -e "QWEN_DSPARK_MODE=$mode"',
        '    -e "QWEN_GDN_DUAL_STATE_COPY=${QWEN_GDN_DUAL_STATE_COPY:-0}" \\\n'
        '    -e "QWEN_DSPARK_MODE=$mode"')
...
def main():
    ...
    if options.manifest.exists() or (scripts / 'dual-state-evidence').exists():
        raise ValueError('Fresh combined copy staging required')
    ...
    for name in ('gdn_dual_state_experiment.py', 'gdn_dual_state_comparison.py', 'gdn_dual_state_gate.py',
    ...
    shutil.copytree(options.evidence, scripts / 'dual-state-evidence')
