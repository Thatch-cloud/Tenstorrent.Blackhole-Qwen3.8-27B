"""Matched complete requests; only draft score selection changes."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('compact_score', {}).get('compact'), value.get('instrumented_timing'))
            for value in requests] != list(SCHEDULE):
        raise ValueError('Two fresh audits followed by complete unchanged/compact/compact/unchanged requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['compact_score']['compact'] is enabled]) for name, enabled in (('unchanged', False), ('compact', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(unchanged_tg=baseline, compact_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['compact']['committed_tg'] / arms['unchanged']['committed_tg'] - 1),
