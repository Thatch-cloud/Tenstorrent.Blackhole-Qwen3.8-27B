...
class CenterFillTests(unittest.TestCase):
    def test_hardware_workflow_flags(self):
        ...
        inputs = SimpleNamespace(suite='dspark-center-tile-fill-hardware')
        condition = step.split('        if: ', 1)[1].splitlines()[0]
        self.assertTrue(eval(condition.replace('&&', ' and ').replace('||', ' or '),
            {'__builtins__': {}}, {'inputs': inputs}))
        expected = {'QWEN_DSPARK_CENTER_TILE_FILL': '1', 'QWEN_DSPARK_NORMALIZATION_DIRECT_STAGE': '1',
            'QWEN_DSPARK_DIRECT_FP32_STAGE': '1', 'QWEN_DSPARK_SFPU_NUMERICAL': '1',
