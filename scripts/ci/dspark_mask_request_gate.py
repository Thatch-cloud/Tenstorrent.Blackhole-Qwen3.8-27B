"""64K mask fast-path numerical evidence required before combined requests."""
...
RUN_ID = 34825617040
REPORT_SHA256 = 'e71363ba9c8823f11bd9fe588819ed8d6fa0e5876276fa08c165140f27d0fe87'


def qualify(directory, report_path):
    payload = Path(report_path).read_bytes()
    if hashlib.sha256(payload).hexdigest() != REPORT_SHA256:
        raise ValueError('Exact completed mask fast-path hardware numerical evidence required')
    report = json.loads(payload)
    expected = dict(passed=True, closed_cleanly=True, backend='hardware', stage='complete',
        candidate='bitwise-negative-infinity-mask', capacity=66560, positions=[65536, 66545],
    ...
    for name, count in counts.items():
        ...
        if len(report.get(name, [])) != count or any(value.get(flag) is not True for value in report[name]):
            raise ValueError('Missing or failed mask fast-path checks: ' + name)
