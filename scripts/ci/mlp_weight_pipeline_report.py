"""Independently recompute complete-request weight-pipeline comparison evidence."""
...
def validate_route(request, arm):
    identity = request.get('weight_weight_pipeline', {})
...
def validate(report):
    ...
    for before, after in (('sources', 'sources_after'), ('native_sources', 'native_sources_after'),
            ('weight_pipeline_sources', 'weight_pipeline_sources_after')):
    ...
    if result != report.get('weight_pipeline_comparison'):
