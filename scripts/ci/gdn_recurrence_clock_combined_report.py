...
def validate(report):
    ...
    for before, after in (('sources', 'sources_after'), ('native_sources', 'native_sources_after'),
            ('gdn_recurrence_clock_sources', 'gdn_recurrence_clock_sources_after')):
    ...
    summary = report.get('gdn_recurrence_clock_combined', {})
    norm, history = request.get('gdn_norm_prefetch', {}), request.get('incremental_history', {})
    builds = summary.get('builds', [])
    if (summary != request.get('gdn_recurrence_clock_combined')
