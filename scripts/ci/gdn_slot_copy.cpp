...
void transfer(uint32_t argument_offset, uint32_t worker, uint32_t scratch) {
    ...
    const uint32_t source_slot_offset = get_arg_val<uint32_t>(argument_offset + 3);
    const uint32_t destination_slot_offset = get_arg_val<uint32_t>(argument_offset + 4);
    for (uint32_t page = worker; page < pages; page += 48) {
        if constexpr (row_only) {
            noc_async_read(source.get_noc_addr(page, source_slot_offset), scratch, 32);
            noc_async_read(source.get_noc_addr(page, 512 + source_slot_offset), scratch + 512, 32);
            noc_async_read_barrier();
            noc_async_write(scratch, destination.get_noc_addr(page, destination_slot_offset), 32);
            noc_async_write(scratch + 512, destination.get_noc_addr(page, 512 + destination_slot_offset), 32);
        } else {
            noc_async_read_tile(page + source_slot_offset, source, scratch);
            noc_async_read_barrier();
            noc_async_write_tile(page + destination_slot_offset, destination, scratch);
        }
