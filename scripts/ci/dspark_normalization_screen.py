...
from dspark_normalization_request_gate import qualify
...
def summarize_screen(requests, *, expected, admission, sources):
    ...
    result.update(direct_fp32_stage=True, normalization_direct_stage=True, normalization_hardware_admission=admission,
...
def screen_scope(directory):
    ...
    admission = qualify(directory, directory / 'dspark-normalization-direct-stage-hardware.json')
    dependencies = ('dspark_direct_fp32_stage.py', 'dspark_normalization_direct_stage.py', 'dspark_normalization_request_gate.py', Path(__file__).name)
