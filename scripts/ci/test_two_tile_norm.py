...
class BindingTests(unittest.TestCase):
    def model(self, ttnn, layers=64):
        ...
        args.get_norm_config = Mock(side_effect=lambda name, mode: native_config(ttnn, name=name, mode=mode))
        return SimpleNamespace(args=args, layers=[object()] * layers)

    def test_decode_calls_come_back_two_tiles_high_and_prefill_ones_untouched(self):
        ttnn = FakeTTNN()
    ...
    def test_the_binding_reaches_the_native_getter_through_instance_overrides(self):
        ...
        with instance_overrides([binding.binding]):
...
class ModelBatchWiringTests(unittest.TestCase):
    """model_batch builds the binding only beyond one tile and checks every decode norm took it."""

    def test_within_one_tile_there_is_no_norm_binding_and_the_module_is_never_imported(self):
        import sys
        from unittest.mock import patch

        from model_batch import norm_bindings

        with patch.dict(sys.modules, {'two_tile_norm': None}):
            for rows in (1, 2, 4, 8, 16, 32):
                with self.subTest(rows=rows):
                    self.assertIsNone(norm_bindings(rows, SimpleNamespace(), FakeTTNN()))
        with self.assertRaises(ValueError):
            norm_bindings(48, SimpleNamespace(), FakeTTNN())

    def test_beyond_one_tile_the_binding_is_the_two_tile_norm_one(self):
        from model_batch import norm_bindings

        ttnn = FakeTTNN()
        args = SimpleNamespace(get_norm_config=lambda name, mode: native_config(ttnn))
        model = SimpleNamespace(args=args, layers=[object()] * 64)
        binding = norm_bindings(64, model, ttnn)
        self.assertIsInstance(binding, TwoTileNormBinding)
        self.assertEqual(binding.binding[:2], (args, 'get_norm_config'))
        self.assertEqual(binding.expected_calls, 129)

    def fixture(self, norm_binding):
        ...
        fixture.norm_binding = norm_binding
        fixture.bindings = [] if norm_binding is None else [norm_binding.binding]
        return fixture

    def test_run_demands_every_decode_norm_rebuilt_and_only_for_a_bound_block(self):
        ttnn = FakeTTNN()
        model = SimpleNamespace(args=SimpleNamespace(get_norm_config=lambda name, mode: native_config(ttnn)),
                                layers=[object()] * 3)
        binding = bind_two_tile_norms(model, 64, ttnn)
        fixture = self.fixture(binding)
        seen = []

        def forward(*args, **kwargs):
            fixture.gdn_calls += 48
            for name in ('attn', 'attn', 'attn', 'attn', 'attn', 'attn', 'lm_head'):
                seen.append(model.args.get_norm_config(name, Mode.DECODE)['sharded_program_config'].block_h)
            return 'logits'
        ...
        self.assertEqual(binding.calls, 7)
        # the binding is off again outside the forward
        self.assertEqual(model.args.get_norm_config('attn', Mode.DECODE)['sharded_program_config'].block_h, 1)
        short = Mock(side_effect=lambda *a, **k: (setattr(fixture, 'gdn_calls', fixture.gdn_calls + 48),
                                                   model.args.get_norm_config('attn', Mode.DECODE)))
        fixture.model = SimpleNamespace(_forward_decode=short)
        with self.assertRaisesRegex(AssertionError, 'two-tile config: 1 rebuilt, 7 expected'):
            fixture.run()
        # an unbound (one-tile) fixture asks nothing of the norms
        narrow = self.fixture(None)
