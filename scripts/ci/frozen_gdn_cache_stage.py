...
def adapt_probe(source, *, norm_prefetch=False):
    if type(norm_prefetch) is not bool:
        raise ValueError('Explicit norm-prefetch selection required')
    module = 'frozen_gdn_norm_prefetch' if norm_prefetch else 'frozen_gdn_input_cache'
...
def main():
    ...
    candidate = adapt_probe(original, norm_prefetch=options.norm_prefetch)
    helper_name = 'frozen_gdn_norm_prefetch.py' if options.norm_prefetch else 'frozen_gdn_input_cache.py'
