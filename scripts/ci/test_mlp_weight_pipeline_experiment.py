...
import mlp_weight_pipeline_experiment as experiment
from mlp_weight_pipeline_comparison import SCHEDULE
from mlp_weight_pipeline_gate import REPORT_SHA256
from mlp_weight_pipeline_stage import adapt
from dspark_request_experiment import summarize
from test_mlp_weight_pipeline_comparison import fixture
...
class ReadOrderExperimentTests(unittest.TestCase):
    def test_matched_runtime_routes_restore_between_all_six_requests(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            candidate = directory / 'mlp-weight-pipeline-candidate'
            candidate.mkdir()
            ...
            for name in ('mlp_weight_pipeline_gate.py', 'mlp_weight_pipeline_comparison.py',
                    'mlp_weight_pipeline_experiment.py', 'mlp_weight_pipeline.py'):
            ...
            def original_route(value, arm):
                enabled = value['weight_weight_pipeline']['pipelined']
            ...
            environment = {name: '1' for name in ('QWEN_MLP_WEIGHT_PIPELINE', 'QWEN_FROZEN_COMBINED_RUNTIME',
                'QWEN_CARDS_ALLOCATED', 'QWEN_HARDWARE_TESTS')}
            with patch.dict('sys.modules', modules), patch.dict('os.environ', environment, clear=True), \
                    patch.object(experiment, '__file__', str(directory / 'mlp_weight_pipeline_experiment.py')), \
            ...
            self.assertEqual(report['weight_pipeline_comparison']['arms']['pipelined']['committed_tg'], 120)
            self.assertEqual(report['weight_pipeline_sources'], report['weight_pipeline_sources_after'])
    ...
    def test_stage_schedule_keeps_winning_publication_policy_in_every_arm(self):
        ...
        self.assertIn('QWEN_MLP_WEIGHT_PIPELINE', result['run-dspark-hardware.sh'])
