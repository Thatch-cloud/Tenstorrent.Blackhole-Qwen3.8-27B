"""Experimental T32 long-context folded attention versus exact native B1 SDPA."""
...
def main():
    ...
    report = dict(passed=False, rows=32, backend='hardware' if options.hardware else 'simulator',
        scope='T32 long-context attention component, at CTX4096; not full-model correctness or TG',
        ...
        sources={name: hashlib.sha256(Path(__file__).with_name(name).read_bytes()).hexdigest()
                 for name in ('attention_replay.py', 'attention_mask_replay.py', 'attention_mask_replay.cpp',
                              'attention_parallel.py', 'attention_fold_dma.py', 'attention_fold_dma.cpp', 'target-t32-attention-probe.py')})
    mesh = None
    try:
        ...
        for capacity in (4352,):
            torch.manual_seed(capacity)
            first, rows = capacity - 256, 32
            ...
            try:
                ...
                try:
                    if any(not torch.equal(actual, expected) for actual, expected in zip(host(warm), gold[0], strict=True)):
                        raise AssertionError('T32 long-context warm output differs from native B1')
                finally:
                ...
                for ticket, (start, ticket_query, expected) in enumerate(zip(starts, queries, gold, strict=True)):
                    ...
                    for chip, (actual, reference) in enumerate(zip(host(output), expected, strict=True)):
                        if not torch.equal(actual, reference):
                            ...
                            raise AssertionError(f'T32 replay differs from native B1: capacity={capacity}, start={start}, chip={chip}')
                        if ticket == 1:
                    ...
                    for bundle_index, (bundle, _, mask, _) in enumerate(reader.metadata):
                        ...
                        for chip, actual in enumerate(host(mask)):
                            if not torch.equal(actual, expected_mask):
                                raise AssertionError('Captured T32 long-context causal mask differs from host oracle')
                            report['mask_checks'].append(dict(capacity=capacity, start=start, ticket=ticket,
                                chip=chip, bundle=bundle_index, exact=True))
                    if original_ids != [addresses(ttnn, tensor) for tensor in owned + reader.owned]:
                        raise AssertionError('Captured T32 long-context buffers moved')
                    options.output.write_text(json.dumps(report, indent=2))
        ...
        report['passed'] = (len(report['checks']) == 8 and len(report['mask_checks']) == 24
                            and len(report['source_checks']) == 4 and report['stale_controls'] == 2
                            and report['mask_poison_controls'] == 12)
        if not report['passed']:
            raise AssertionError('Incomplete T32 long-context attention gate')
