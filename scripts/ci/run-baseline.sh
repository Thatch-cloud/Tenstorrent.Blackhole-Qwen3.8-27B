...
[[ "$mode" = baseline || "$mode" = interleave || "$mode" = profile || "$mode" = model-profile || "$mode" = mlp-sweep || "$mode" = mlp-packing || "$mode" = sampling-kernel || "$mode" = sampling || "$mode" = sampling-extended ]]
