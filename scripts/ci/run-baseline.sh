...
[[ "$mode" = baseline || "$mode" = interleave || "$mode" = profile || "$mode" = model-profile || "$mode" = mlp-sweep || "$mode" = mlp-packing || "$mode" = mlp-fusion || "$mode" = projection-1d || "$mode" = full-model-fusion || "$mode" = full-prefix || "$mode" = full-batch || "$mode" = full-gdn-row-layout || "$mode" = full-gdn-device-loop || "$mode" = full-verifier-replay || "$mode" = full-verifier-engine || "$mode" = full-verifier-selection || "$mode" = full-gdn-row-clones || "$mode" = full-gdn-input-reuse || "$mode" = full-compact-gdn || "$mode" = full-coding-cost || "$mode" = full-batch-attribution || "$mode" = attention-batch || "$mode" = attention-timing || "$mode" = gdn-prefix || "$mode" = gdn-block || "$mode" = gdn-active || "$mode" = gdn-multitoken || "$mode" = gdn-value-split || "$mode" = gdn-value-split-timing || "$mode" = gdn-value-split-prefetch || "$mode" = gdn-value-split-norm-batch || "$mode" = gdn-multitoken-norm || "$mode" = gdn-multitoken-conv || "$mode" = device-readback || "$mode" = gdn-checkpoint-dma || "$mode" = gdn-checkpoint-cost || "$mode" = gdn-inplace-timing || "$mode" = gdn-inplace || "$mode" = gdn-direct || "$mode" = sampling-kernel || "$mode" = sampling || "$mode" = sampling-extended ]]
