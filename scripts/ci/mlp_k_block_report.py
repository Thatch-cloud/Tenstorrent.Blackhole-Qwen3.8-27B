"""Independently recompute complete-request k-block comparison evidence."""
...
def validate_route(request, arm):
    identity = request.get('mlp_k_block', {})
...
def validate(report):
    ...
    for before, after in (('sources', 'sources_after'), ('native_sources', 'native_sources_after'),
            ('k_block_sources', 'k_block_sources_after')):
    ...
    if result != report.get('k_block_comparison'):
