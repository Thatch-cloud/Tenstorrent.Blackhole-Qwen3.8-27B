...
from gdn_window_write_comparison import SCHEDULE, summarize
from gdn_window_write_gate import qualify, REPORT_SHA256
from gdn_window_write_scope import scoped_window_writes


FILES = ('gdn_window_write_experiment.py', 'gdn_window_write_comparison.py', 'gdn_window_write_gate.py',
    'gdn_window_write_scope.py', 'gdn_window_write_overlap.py', 'gdn-window-write-probe.py',
...
def run_loaded_requests(operations, generator, model, collectives, tokenizer, pages, kv_cache, parameters,
    ...
    if (any(os.environ.get(name) != '1' for name in ('QWEN_GDN_WINDOW_WRITE',
    ...
    evidence = qualify(directory, directory / 'gdn-window-write-evidence')
    ...
    def measure(*args, **kwargs):
        if len(calls) >= len(SCHEDULE):
            raise ValueError('Unexpected additional gdn-window-write request')
    ...
    def finish(result, summarize_requests):
        ...
        result.update(gdn_window_write_comparison=comparison, pp=None, committed_tg=None,
            ctx_tokens=4096, fresh_context_audit=True,
            scope='Same winning T16/HiFi4 runtime; convolution-window write scheduling only')
    ...
    try:
        ...
        report['gdn_window_write_sources'] = sources
        report['gdn_window_write_sources_after'] = fingerprints()
        if (report['gdn_window_write_sources_after'] != sources
                or qualify(directory, directory / 'gdn-window-write-evidence') != evidence):
