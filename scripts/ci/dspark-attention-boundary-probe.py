"""Tiny draft and folded target attention compiled under the same guarded headers."""
...
def main():
    ...
    with boundary_scope(os.environ['TT_METAL_HOME']), mask_scope(), sum_scope(), small_score_fixture(), bitwise_infinity_checks(), factory_scope(), kernel_scope(), \
    ...
    report.update(candidate='draft-target-header-boundary', performance_qualified=False,
        ...
        candidate_sources={name: hashlib.sha256((directory / name).read_bytes()).hexdigest()
            for name in ('dspark_score_bitwise.py', 'dspark_score_smoke_geometry.py',
                ...
                'dspark_sum_sfpu_build.py', 'dspark_mask_bits.py', 'dspark_attention_header_boundary.py', Path(__file__).name)})
