"""Matched complete requests; only the T16 MLP reduction block width changes."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('mlp_k_block', {}).get('k32'), value.get('instrumented_timing'))
            for value in requests] != list(SCHEDULE):
        raise ValueError('Two fresh audits followed by complete unchanged/k32/k32/unchanged requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['mlp_k_block']['k32'] is enabled]) for name, enabled in (('unchanged', False), ('k32', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(unchanged_tg=baseline, k32_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['k32']['committed_tg'] / arms['unchanged']['committed_tg'] - 1),
