"""Matched complete requests; only direct convolution-window construction changes."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('gdn_direct_window', {}).get('direct'), value.get('instrumented_timing'))
        ...
        raise ValueError('Two fresh audits followed by complete unchanged/direct/direct/unchanged requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['gdn_direct_window']['direct'] is enabled]) for name, enabled in (('unchanged', False), ('direct', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(unchanged_tg=baseline, direct_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['direct']['committed_tg'] / arms['unchanged']['committed_tg'] - 1),
