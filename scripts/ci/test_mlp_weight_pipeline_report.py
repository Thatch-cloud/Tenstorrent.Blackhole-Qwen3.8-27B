...
from mlp_weight_pipeline_comparison import summarize
from mlp_weight_pipeline_report import validate, validate_audit, validate_route, summarize_requests
from mlp_weight_pipeline_report import BASELINE_SHA256, REPORT_SHA256, NORM_SHA256, HISTORY_SHA256, TAPS
from test_mlp_weight_pipeline_comparison import fixture as request_fixture


def fixture():
    requests = request_fixture()
    for request in requests:
        enabled = request['weight_weight_pipeline']['pipelined']
        request['weight_weight_pipeline']['simulator_report_sha256'] = REPORT_SHA256 if enabled else None
    ...
    return dict(passed=True, closed_cleanly=True, stage='complete', streams=1, ctx_tokens=4096,
        sampler_links=4, fresh_context_audit=True, request_checks=requests, weight_pipeline_comparison=comparison,
        sources={'one': 'a' * 64}, sources_after={'one': 'a' * 64},
        native_sources={'two': 'b' * 64}, native_sources_after={'two': 'b' * 64},
        weight_pipeline_sources={'three': 'c' * 64}, weight_pipeline_sources_after={'three': 'c' * 64})


class ReadOrderReportTests(unittest.TestCase):
    def test_complete_report_recomputes_tg(self):
        self.assertEqual(validate(fixture())['arms']['pipelined']['committed_tg'], 120)
    ...
    def test_missing_evidence_or_forged_rate_rejected(self):
        for mutate in (
                ...
                lambda report: report['weight_pipeline_comparison']['arms']['pipelined'].update(committed_tg=200)):
            report = fixture()
            mutate(report)
