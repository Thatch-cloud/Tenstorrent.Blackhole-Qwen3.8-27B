...
def adapt(source):
    source = replace_once(source, 'from gdn_shared_qk_pipeline import build as build_pipeline',
        'from gdn_wait_clock_pipeline import build as build_pipeline, BUILD_RECORDS\n'
    ...
    source = replace_once(source,
        ...
        'rows=16, norm_unchanged=True, state_math_unchanged=True, shared_qk_preparation=True,\n'
        '        wait_clock=True, generated_kernels=BUILD_RECORDS, committed_tg=None)')
    source = replace_once(source, '                try:\n                    def allocate(',
        '                try:\n'
        '                    self.samples = WaitClockCapture(ttnn, mesh, self.owned)\n'
    ...
    source = replace_once(source, "        report['passed'] = True",
        "        report['wait_clock_samples'] = candidate.samples.records\n"
...
def main():
    ...
    for before_name, after_name, transform in (
            ('gdn_shared_qk_pipeline.py', 'gdn_wait_clock_pipeline.py', instrument_pipeline),
