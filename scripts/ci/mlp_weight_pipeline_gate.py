"""Exact simulator and source admission for a weight-pipeline-only T16 candidate."""

import hashlib
import json
from pathlib import Path

from mlp_weight_pipeline import transform


REPORT_SHA256 = 'abbb01d1ece1b625c19909263c9a45eab5fa859d46d27e265cd84e1535166371'
...
def qualify(directory, evidence, expected_report_sha256=REPORT_SHA256):
    ...
    if hashlib.sha256(raw).hexdigest() != expected_report_sha256:
        raise ValueError('Exact reviewed weight-pipeline simulator report required')
    ...
    for name in ('fused_1d.py', 'fused_1d_input.cpp'):
        if (candidate / name).read_bytes() != (directory / name).read_bytes():
            raise ValueError('Only weight-reader DMA scheduling may differ: ' + name)
    ...
    if (candidate / 'fused_1d_weights.cpp').read_bytes() != changed:
        raise ValueError('Exact reviewed two-transaction pipeline required')
