...
def patch_scheduler(source):
    ...
    lines = replace_once(
        ...
        '        # "A prefill is in flight" cannot be read off is_prefill_chunk alone: it\n'
        '        # is false before the first chunk and after the last, and run 35714211185\n'
        '        # died in one of those gaps. The lifecycle publishes the authoritative\n'
        '        # answer; absent or unreadable means not held, which is prior behaviour.\n'
        '        import sys as _qwen_sys\n'
        '        try:\n'
        '            _qwen_gate = getattr(\n'
        '                _qwen_sys.modules.get("_qwen_prefill_gate"), "held", None)\n'
        '        except BaseException:\n'
        '            _qwen_gate = None\n'
        '        if partial_prefills:\n'
        '            _qwen_allowed = len(partial_prefills)\n'
        '        elif _qwen_gate is not None:\n'
        '            _qwen_allowed = 0\n'
        '        else:\n'
        '            _qwen_allowed = 1\n'
        '        _qwen_hide = bool(partial_prefills) or _qwen_gate is not None\n'
        ...
        '            f"hidden={_qwen_hide} gate={_qwen_gate!r}")\n'
        '        try:\n'
        '            result = super().schedule()\n'
        '        finally:\n'
        '            if _qwen_hide:\n'
