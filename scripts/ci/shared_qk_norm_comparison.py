"""Matched complete requests; prefetched versus direct-scatter T16 normalization."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('norm_reader', {}).get('scatter'), value.get('instrumented_timing'))
            for value in requests] != list(SCHEDULE):
        raise ValueError('Two fresh audits followed by complete prefetched/scatter/scatter/prefetched requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['norm_reader']['scatter'] is enabled]) for name, enabled in (('prefetched', False), ('scatter', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(prefetched_tg=baseline, scatter_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['scatter']['committed_tg'] / arms['prefetched']['committed_tg'] - 1),
