...
from mlp_k_block_comparison import summarize
from mlp_k_block_report import validate, validate_audit, validate_route, summarize_requests
from mlp_k_block_report import BASELINE_SHA256, REPORT_SHA256, NORM_SHA256, HISTORY_SHA256, TAPS
from test_mlp_k_block_comparison import fixture as request_fixture


def fixture():
    requests = request_fixture()
    for request in requests:
        enabled = request['mlp_k_block']['k32']
        request['mlp_k_block']['simulator_report_sha256'] = REPORT_SHA256 if enabled else None
    ...
    return dict(passed=True, closed_cleanly=True, stage='complete', streams=1, ctx_tokens=4096,
        sampler_links=4, fresh_context_audit=True, request_checks=requests, k_block_comparison=comparison,
        sources={'one': 'a' * 64}, sources_after={'one': 'a' * 64},
        native_sources={'two': 'b' * 64}, native_sources_after={'two': 'b' * 64},
        k_block_sources={'three': 'c' * 64}, k_block_sources_after={'three': 'c' * 64})


class ReadOrderReportTests(unittest.TestCase):
    def test_complete_report_recomputes_tg(self):
        self.assertEqual(validate(fixture())['arms']['k32']['committed_tg'], 120)
    ...
    def test_missing_evidence_or_forged_rate_rejected(self):
        for mutate in (
                ...
                lambda report: report['k_block_comparison']['arms']['k32'].update(committed_tg=200)):
            report = fixture()
            mutate(report)
