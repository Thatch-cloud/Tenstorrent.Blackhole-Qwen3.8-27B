"""Matched complete requests; only GDN input gathering order changes."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('gdn_input_overlap', {}).get('overlapped_inputs'), value.get('instrumented_timing'))
            for value in requests] != list(SCHEDULE):
        raise ValueError('Two fresh audits followed by complete unchanged/overlapped_inputs/overlapped_inputs/unchanged requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['gdn_input_overlap']['overlapped_inputs'] is enabled]) for name, enabled in (('unchanged', False), ('overlapped_inputs', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(unchanged_tg=baseline, overlapped_inputs_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['overlapped_inputs']['committed_tg'] / arms['unchanged']['committed_tg'] - 1),
