...
from gdn_qk_double_buffer_comparison import SCHEDULE, summarize
from gdn_qk_double_buffer_gate import qualify, REPORT_SHA256, KERNEL
from gdn_qk_double_buffer_scope import scoped_double_buffer


FILES = ('gdn_qk_double_buffer_experiment.py', 'gdn_qk_double_buffer_comparison.py', 'gdn_qk_double_buffer_gate.py',
...
def run_loaded_requests(operations, generator, model, collectives, tokenizer, pages, kv_cache, parameters,
    ...
    if (any(os.environ.get(name) != '1' for name in ('QWEN_GDN_QK_DOUBLE_BUFFER',
    ...
    evidence = qualify(directory / 'qk-double-buffer-evidence', directory, os.environ['TT_METAL_HOME'])
    ...
    def measure(*args, **kwargs):
        if len(calls) >= len(SCHEDULE):
            raise ValueError('Unexpected additional qk-double-buffer request')
        ...
        if (kwargs.get('audit_features') is not audited
            ...
            raise ValueError('Both Q/K-buffer arms must retain the complete winning T16 runtime')
        calls.append((enabled, audited))
        with (scoped_double_buffer(evidence) if enabled else nullcontext()) as audit:
        ...
        result['gdn_qk_double_buffer'] = dict(double_buffered_qk=enabled, builds=builds,
    ...
    def finish(result, summarize_requests):
        ...
        result.update(qk_double_buffer_comparison=comparison, pp=None, committed_tg=None,
            ctx_tokens=4096, fresh_context_audit=True,
            scope='Same winning T16/HiFi4 runtime; GDN normalized Q/K ring capacity only')
    ...
    try:
        ...
        report['qk_double_buffer_sources'] = sources
        report['qk_double_buffer_sources_after'] = fingerprints()
        if (report['qk_double_buffer_sources_after'] != sources
                or qualify(directory / 'qk-double-buffer-evidence', directory, os.environ['TT_METAL_HOME']) != evidence):
