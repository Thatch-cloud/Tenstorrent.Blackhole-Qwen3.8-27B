...
from gdn_input_overlap_gate import KERNEL, REPORT_SHA256


@contextmanager
def scoped_overlap(admission):
    import gdn_shared_qk_pipeline as pipeline
    if admission.get('report_sha256') != REPORT_SHA256 or admission.get('kernel') != KERNEL:
        raise ValueError('Source-qualified input-overlap admission required')
    original = pipeline.load_kernels
    if getattr(original, '_input_overlap_copy', False):
        raise ValueError('Nested input-overlap scopes are forbidden')
    ...
    kernels._input_overlap_copy = True
