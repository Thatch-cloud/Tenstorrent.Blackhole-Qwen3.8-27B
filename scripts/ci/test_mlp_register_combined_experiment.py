...
import mlp_register_combined_experiment as experiment
from mlp_register_combined_comparison import SCHEDULE
from mlp_register_epilogue_gate import REPORT_SHA256
from mlp_register_combined_stage import adapt
from dspark_request_experiment import summarize
from test_mlp_register_combined_comparison import fixture
...
class ReadOrderExperimentTests(unittest.TestCase):
    def test_matched_runtime_routes_restore_between_all_six_requests(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for name in ('mlp_register_epilogue_gate.py', 'mlp_register_combined_comparison.py',
                    'mlp_register_combined_experiment.py', 'mlp_register_epilogue.py', 'mlp_rounding_policy.py',
            ...
            def original_route(value, arm):
                enabled = value['register_epilogue']['register_resident']
            ...
            environment = {name: '1' for name in ('QWEN_MLP_REGISTER_EPILOGUE', 'QWEN_FROZEN_COMBINED_RUNTIME',
                'QWEN_CARDS_ALLOCATED', 'QWEN_HARDWARE_TESTS')}
            with patch.dict('sys.modules', modules), patch.dict('os.environ', environment, clear=True), \
                    patch.object(experiment, '__file__', str(directory / 'mlp_register_combined_experiment.py')), \
            ...
            self.assertEqual(report['register_epilogue_comparison']['arms']['register_resident']['committed_tg'], 120)
            self.assertEqual(report['register_epilogue_sources'], report['register_epilogue_sources_after'])
    ...
    def test_stage_schedule_keeps_winning_publication_policy_in_every_arm(self):
        ...
        self.assertIn('QWEN_MLP_REGISTER_EPILOGUE', result['run-dspark-hardware.sh'])
