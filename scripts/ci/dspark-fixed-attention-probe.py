"""Fixed physical proposal storage with changing full-history frontier; no target model or hardware-TG claim."""
...
from dspark_full_attention import execute, geometry
...
SOURCES = tuple(sorted(set(FULL.SOURCES + ('dspark-fixed-attention-probe.py', 'dspark_fixed_inputs.py', 'dspark_cached_layer.py'))))
CAPACITY, PROPOSALS = 4384, 15
POSITIONS = (4096, 4109)
REPLAYS = (1, 0)
...
def main():
    ...
    if (options.output.exists() or os.environ.get('QWEN_SIM_SHARED_BDF') != '1'
        ...
        raise ValueError('Fresh bounded two-chip simulator gate and unchanged native runtime required')
    import torch
    import ttnn
    ...
    report = dict(passed=False, closed_cleanly=False, backend='simulator', scope=__doc__,
        ...
        sources=source_hashes(), native_sources=FULL.NOISE.fingerprints(root), resources_before=snapshot(bounded=True),
    ...
    try:
        ...
        try:
            ...
            report['sources_after'], report['native_sources_after'] = source_hashes(), FULL.NOISE.fingerprints(root)
