"""Independently recompute complete-request register-epilogue comparison evidence."""
...
def validate_route(request, arm):
    ...
    if (type(enabled) is not bool or arm != 'publication'
        ...
        raise ValueError('Source-bound epilogue identity required')
...
def validate(report):
    ...
    for before, after in (('sources', 'sources_after'), ('native_sources', 'native_sources_after'),
            ('register_epilogue_sources', 'register_epilogue_sources_after')):
    ...
    if result != report.get('register_epilogue_comparison'):
