...
def transform(source, verifier=False):
    ...
    selection = (
        "    generation = json.loads(Path('/experiment/results/full-gdn-device-loop.json').read_text())\n"
        "    if not generation['passed'] or not generation.get('instrumented_timing'):\n"
        "        raise AssertionError('Instrumented verifier oracle failed')\n"
        "    selected = {str(record['trace_id']) for timing in generation['timings']\n"
        "                for record in timing['device_profile']['records']}\n"
        "    if len(selected) != 6:\n"
        "        raise AssertionError('Expected native/control/candidate traces at two contexts')\n"
        "    ops = {key: op for key, op in ops.items() if str(op.get('metal_trace_id')) in selected}\n"
    ) if verifier else (
...
def main():
    ...
    updated = transform(source, verifier=os.environ.get('QWEN_PROFILE_VERIFIER') == '1')
    ast.parse(updated)
    path = root / relative
