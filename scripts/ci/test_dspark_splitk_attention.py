...
class SplitKAdapterTests(unittest.TestCase):
    def test_device_layout_matches_oracle_without_copying_kv(self):
        self.check_layout(256, False)

    def test_striped_partitions_preserve_attention(self):
        self.check_layout(512, True)

    def check_layout(self, storage, stripe):
        torch.manual_seed(3)
        query = torch.randn(1, 16, 32, 128)
        keys, values = torch.randn(1, 4, storage, 128), torch.randn(1, 4, storage, 128)
        mask = torch.zeros(1, 1, 32, storage)
        mask[..., 128:storage - 16] = float('-inf')
        calls = []
        ...
        def decode(folded, actual_keys, actual_values, **options):
            ...
            self.assertEqual(tuple(actual_keys.shape), (4, 1, storage, 128))
            ...
            if stripe:
                chunks = options['attn_mask'].reshape(4, 1, 128, 16, 32)
                self.assertTrue(bool(torch.isfinite(chunks).any(dim=-1).all()))
        ...
        actual = execute_folded(operations, query, keys, values, mask, [], stripe_keys=stripe)
