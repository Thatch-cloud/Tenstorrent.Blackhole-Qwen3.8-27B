...
import compact_score_experiment as experiment
from dspark_request_experiment import summarize
from test_compact_score_comparison import fixture
    ...
    def test_all_six_requests_keep_norm_prefetch_and_restore_routes(self):
        with TemporaryDirectory() as temporary:
            ...
            def scope(evidence, directory):
                audit = dict(calls=96, restored=False)
                active.append(True)
                try:
                    yield audit
                finally:
            ...
            def measure(*args, **kwargs):
                ...
                result['score_layout'] = dict(calls=96)
                return result
            ...
            environment = {name: '1' for name in ('QWEN_COMPACT_SCORE_HARDWARE', 'QWEN_FROZEN_COMBINED_RUNTIME',
            ...
            with patch.dict('sys.modules', modules), patch.dict('os.environ', environment, clear=True), \
                    patch.object(experiment, '__file__', str(directory / 'compact_score_experiment.py')), \
                    patch.object(experiment, 'qualify', return_value={}), patch.object(experiment, 'scoped_compact_scores', scope):
            ...
            self.assertEqual(len(report['compact_route_diagnostics']), 6)
            self.assertEqual([entry['score_layout_calls'] for entry in report['compact_route_diagnostics']], [96] * 6)
            self.assertEqual(report['compact_score_comparison']['arms']['compact']['committed_tg'], 120)
            self.assertEqual(report['compact_score_sources'], report['compact_score_sources_after'])
