...
def stage(checkout, manifest, *, diagnose_activation=False, diagnose_rounding=False):
    if manifest.exists():
    ...
    payloads = {'fused_1d.py': adapt_projection(originals['fused_1d.py'],
            diagnose_activation=diagnose_activation, diagnose_rounding=diagnose_rounding),
...
def main():
    ...
    stage(options.checkout, options.manifest, diagnose_activation=options.diagnose_activation,
        diagnose_rounding=options.diagnose_rounding)
