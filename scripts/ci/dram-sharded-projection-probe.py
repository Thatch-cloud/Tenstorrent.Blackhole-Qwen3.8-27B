"""Simulator-only native DRAM-sharded T16 gate projection screen; no speed claim."""
...
def main():
    ...
    names = ('dram-sharded-projection-probe.py', 'dram_sharded_projection.py')
    def hashes():
    ...
    try:
        ...
        config = configurations(ttnn, mesh, 'gate')
        report['geometry'] = config['plan']
        generator = torch.Generator().manual_seed(383950)
        host_weight = (torch.randint(-8, 9, (2, 1, 5120, 8704), generator=generator).float() / 32).bfloat16()
        mapper = ttnn.ShardTensorToMesh(mesh, dim=0)
        def retain(value):
            owned.append(value)
            return value
        weight = retain(ttnn.from_torch(host_weight, device=mesh, dtype=ttnn.bfloat4_b,
        ...
        control = create_matmul_1d_decode_progcfg(16, 5120, 8704, 44,
            fused_activation=ttnn.UnaryOpType.SILU, fp32_acc=True, grid_w=11)
        for pattern in range(2):
            save('eager_' + str(pattern))
            host = (torch.randn((2, 1, 16, 5120), generator=generator) * .125).bfloat16()
            ...
            candidate = execute(ttnn, source, sharded, config, compute, retain)
            for chip in range(2):
