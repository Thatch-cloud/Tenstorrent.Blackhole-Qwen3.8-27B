...
import mlp_clock_experiment as experiment
from mlp_clock_combined_stage import adapt
...
class ClockExperimentTests(unittest.TestCase):
    def test_one_audited_request_preserves_existing_route_and_releases_after_scope(self):
        ...
        environment = {name: '1' for name in ('QWEN_MLP_CLOCK_COMBINED', 'QWEN_FROZEN_COMBINED_RUNTIME',
        ...
        with patch.dict('sys.modules', modules), patch.dict('os.environ', environment, clear=True), \
                ...
                patch.object(experiment, 'ClockCapture', capture), patch.object(experiment, 'CombinedClockCapture', Bank):
        ...
        self.assertEqual(report['mlp_clock_combined']['layers'], 64)
        ...
        self.assertEqual(report['mlp_clock_sources'], report['mlp_clock_sources_after'])
    ...
    def test_staging_only_replaces_request_schedule_and_entrypoint(self):
        ...
        self.assertIn('from mlp_clock_experiment import run_loaded_requests', result['dspark-target-hardware.py'])
        self.assertIn('QWEN_MLP_CLOCK_COMBINED', result['run-dspark-hardware.sh'])
