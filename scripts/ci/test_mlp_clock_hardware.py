...
import mlp_clock_hardware
from mlp_clock_hardware import retained, verify_sources
...
class ClockHardwareTests(unittest.TestCase):
    def test_source_tampering_and_path_escape_rejected(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (directory / 'clock-hardware-sources.json').write_text(json.dumps(manifest))
            verify_sources(directory)
            (directory / 'candidate.py').write_bytes(b'changed source')
            with self.assertRaises(ValueError):
                verify_sources(directory)
            (directory / 'clock-hardware-sources.json').write_text(json.dumps({'../outside': 'none'}))
            with self.assertRaises(ValueError):
                verify_sources(directory)
    ...
    def test_retained_report_requires_hash_exit_runtime_and_cleanup(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(ValueError):
                retained(directory)
            with patch.object(mlp_clock_hardware, 'SIMULATOR_SHA256', hashlib.sha256(raw).hexdigest()):
