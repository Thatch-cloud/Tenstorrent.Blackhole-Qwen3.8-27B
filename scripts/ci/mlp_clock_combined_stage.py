...
from mlp_clock_experiment import HELPERS, candidate_class
...
def adapt(sources):
    ...
    result['dspark-target-hardware.py'] = replace_once(result['dspark-target-hardware.py'],
        ...
        '            from mlp_clock_experiment import run_loaded_requests')
    result['run-dspark-hardware.sh'] = replace_once(result['run-dspark-hardware.sh'],
        '    -e "QWEN_DSPARK_MODE=$mode"',
        '    -e "QWEN_MLP_CLOCK_COMBINED=${QWEN_MLP_CLOCK_COMBINED:-0}" \\\n'
        '    -e "QWEN_DSPARK_MODE=$mode"')
...
def main():
    ...
    if options.manifest.exists() or (scripts / 'clock-evidence').exists():
    ...
    for name in (*HELPERS, 'mlp_clock_experiment.py', 'mlp_clock_combined.py', 'mlp_clock_hardware.py', 'mlp_clock_report.py'):
        result[name] = Path(__file__).with_name(name).read_text()
    for name, source in result.items():
        (scripts / name).write_bytes(source.encode())
    shutil.copytree(options.evidence, scripts / 'clock-evidence')
    candidate_class(scripts, scripts / 'clock-evidence')
