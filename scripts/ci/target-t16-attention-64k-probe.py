...
from target_t16_64k_geometry import geometry, geometry_scope
...
def main():
    ...
    report = dict(passed=False, backend='hardware' if options.hardware else 'simulator',
        scope='T16 long-context attention component, at explicit tiny simulator or CTX65536 hardware geometry; not full-model correctness or TG',
        ...
        sources={name: hashlib.sha256(Path(__file__).with_name(name).read_bytes()).hexdigest()
                 for name in ('attention_replay.py', 'attention_mask_replay.py', 'attention_mask_replay.cpp',
                              'attention_parallel.py', 'attention_fold_dma.py', 'attention_fold_dma.cpp', 'target-t16-attention-64k-probe.py', 'target_t16_64k_geometry.py')})
    mesh = None
    try:
        ...
        for capacity in (geometry(hardware=options.hardware)['capacity'],):
...
if __name__ == '__main__':
    with geometry_scope(hardware='--hardware' in sys.argv):
        main()
