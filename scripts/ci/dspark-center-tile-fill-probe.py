...
def main():
    if (os.environ.get('QWEN_SIM_ONLY') != '1' or not os.environ.get('TT_METAL_SIMULATOR')
            ...
            or os.environ.get('QWEN_CENTER_TILE_FILL') != '1'
    ...
    with center_fill_scope(), normalization_stage_scope(), staging_scope(diagnostic=True), boundary_scope(os.environ['TT_METAL_HOME']), mask_scope(), sum_scope(), small_score_fixture(), bitwise_infinity_checks(), factory_scope(), kernel_scope(), \
    ...
    report.update(candidate='center-tile-fill', performance_qualified=False,
        ...
        candidate_sources={name: hashlib.sha256((directory / name).read_bytes()).hexdigest()
            for name in ('dspark_score_bitwise.py', 'dspark_score_smoke_geometry.py',
                ...
                'dspark_sum_sfpu_build.py', 'dspark_mask_bits.py', 'dspark_attention_header_boundary.py', 'dspark_direct_fp32_stage.py', 'dspark_normalization_direct_stage.py', 'dspark_center_tile_fill.py', Path(__file__).name)})
