...
import gdn_grouped_gather_scope as scope
from gdn_grouped_gather import reader
from gdn_grouped_gather import ORIGINAL


class GroupedGatherScopeTests(unittest.TestCase):
    def test_reader_only_composition_with_norm_prefetch_and_failure_restoration(self):
        ...
        for failed in (False, True):
            ...
            with patch.dict('sys.modules', {'gdn_shared_qk_pipeline': pipeline}), patch.object(scope, 'KERNEL', kernel):
                try:
                    with scope.scoped_gather(dict(report_sha256=scope.REPORT_SHA256, kernel=kernel)) as audit:
