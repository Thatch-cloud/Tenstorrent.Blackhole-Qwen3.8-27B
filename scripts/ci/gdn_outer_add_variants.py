"""Matched combined requests changing only T16 recurrence outer-product/state-add fusion."""
...
from gdn_outer_add_gate import REPORT_SHA256


POLICIES = {'control': dict(PUBLICATION['publication']),
    'publication': dict(PUBLICATION['publication'], gdn_outer_add=True)}


def validate_route(value, arm):
    if arm not in POLICIES:
        raise ValueError('Unknown outer-add arm')
    publication_route(value, 'publication')
    evidence = value.get('gdn_outer_add')
    if arm == 'control':
        if evidence is not None:
            raise ValueError('Native control must not install the outer-add scope')
        return
    if (not isinstance(evidence, dict) or evidence.get('restored') is not True
        ...
        raise ValueError('Qualified and restored outer-add scope required')
