"""Matched complete requests; only native MLP-down output distribution changes."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('mlp_down_grid', {}).get('wider_down'), value.get('instrumented_timing'))
            for value in requests] != list(SCHEDULE):
        raise ValueError('Two fresh audits followed by complete unchanged/wider_down/wider_down/unchanged requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['mlp_down_grid']['wider_down'] is enabled]) for name, enabled in (('unchanged', False), ('wider_down', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(unchanged_tg=baseline, wider_down_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['wider_down']['committed_tg'] / arms['unchanged']['committed_tg'] - 1),
