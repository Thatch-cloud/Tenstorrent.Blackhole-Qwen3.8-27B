...
PY
    ...
    if [[ "$QWEN_SIM_CASE" = gdn-output-* || "$QWEN_SIM_CASE" = gdn-copy-pairs ]]; then
        [[ "$QWEN_SIM_CASE" = gdn-output-l1 || "$QWEN_SIM_CASE" = gdn-output-grid || "$QWEN_SIM_CASE" = gdn-copy-pairs ]]
        status=0
        limit=3600
        if [[ "$QWEN_SIM_CASE" = gdn-copy-pairs ]]; then limit=900; fi
