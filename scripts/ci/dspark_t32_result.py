...
def validate(report):
    if (report.get('passed') is not True or report.get('closed_cleanly') is not True
            ...
            or report.get('proposals') != 31 or report.get('vocabulary') != 64
    ...
    coverage('eager_checks', ('pattern', 'chip', 'step'),
        {(pattern, chip, step) for pattern in range(3) for chip in range(2) for step in range(31)},
        ('token_exact', 'full_vocabulary_exact'))
    coverage('replay_checks', ('repetition', 'pattern', 'chip', 'step'),
        {(repetition, pattern, chip, step) for repetition, pattern in enumerate((0, 1, 2, 0))
    ...
    coverage('input_checks', ('phase', 'ordinal', 'tensor', 'chip'),
        {(phase, ordinal, tensor, chip) for phase, count in (('eager', 3), ('replay', 4))
    ...
    return dict(passed=True, eager_queries=186, replay_queries=248, target_integrated=False,
        scope='Synthetic 64-token vocabulary only; no throughput or full-model qualification')
