"""Independently recompute combined GDN Q/K-buffer correctness and full-cycle TG."""
...
def validate_route(request, arm):
    ...
    identity = request.get('gdn_qk_double_buffer', {})
    enabled, builds = identity.get('double_buffered_qk'), identity.get('builds')
...
def validate(report):
    ...
    for before, after in (('sources', 'sources_after'), ('native_sources', 'native_sources_after'),
            ('qk_double_buffer_sources', 'qk_double_buffer_sources_after')):
    ...
    if comparison != report.get('qk_double_buffer_comparison'):
