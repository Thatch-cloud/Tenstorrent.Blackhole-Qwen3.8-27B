"""Matched complete requests; only convolution-window write scheduling changes."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('gdn_window_write', {}).get('overlap'), value.get('instrumented_timing'))
            for value in requests] != list(SCHEDULE):
        raise ValueError('Two fresh audits followed by complete unchanged/overlap/overlap/unchanged requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['gdn_window_write']['overlap'] is enabled]) for name, enabled in (('unchanged', False), ('overlap', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(unchanged_tg=baseline, overlap_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['overlap']['committed_tg'] / arms['unchanged']['committed_tg'] - 1),
