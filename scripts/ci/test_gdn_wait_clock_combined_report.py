...
from gdn_wait_clock_combined_report import LAYERS, NORM_SHA256, HISTORY_SHA256, validate
from gdn_wait_clock_gate import KERNEL, REPORT_SHA256
from test_gdn_wait_clock_report import fixture as isolated_fixture
...
def fixture():
    ...
    report.update(stage='complete', sampler_links=4,
        gdn_wait_clock_sources={'helper': 'a' * 64},
        gdn_wait_clock_sources_after={'helper': 'a' * 64})
    records = []
    for replay, position in enumerate((4096, 4107)):
        layers = []
        for layer in LAYERS:
            capture = deepcopy(isolated_fixture()['wait_clock_samples'][0])
    ...
    report['gdn_wait_clock_combined'] = summary
    request = report['request_checks'][0]
    request.update(gdn_wait_clock_combined=summary,
...
class CombinedRecurrenceReportTests(unittest.TestCase):
    ...
    def test_incomplete_samples_recipe_changes_and_numerical_failure_rejected(self):
        mutations = (
            ...
            lambda report: report['gdn_wait_clock_sources_after'].clear(),
            ...
            lambda report: report['gdn_wait_clock_combined']['builds'][0]['kernel'].update(token=0),
            lambda report: report['gdn_wait_clock_combined']['records'][0]['layers'].pop(),
            lambda report: report['gdn_wait_clock_combined']['records'][0].update(position=0),
            lambda report: report['gdn_wait_clock_combined']['records'][0]['layers'][0]['capture'].update(poisoned_before_execution=False),
            lambda report: report['gdn_wait_clock_combined']['records'][0]['layers'][0]['capture']['samples'][0].update(duration_cycles=21),
        )
