"""Matched captured DSpark requests changing only the proposal attention backend."""

from dspark_request_variants import proposal_signature


POLICIES = {
    'composed': dict(proposal_trace=True, commit_only_gdn=True, native_attention=False),
    'native': dict(proposal_trace=True, commit_only_gdn=True, native_attention=True),
}
SCHEDULE = (('composed', True), ('native', True), ('composed', False),
    ('native', False), ('native', False), ('composed', False))
...
def summarize_variants(requests):
    ...
    return dict(arms=arms, order=[arm for arm, audit in SCHEDULE],
        committed_tg_change_percent=100 * (arms['native']['committed_tg'] / arms['composed']['committed_tg'] - 1),
