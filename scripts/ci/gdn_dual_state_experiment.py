...
from gdn_dual_state_comparison import SCHEDULE, summarize
from gdn_dual_state_gate import qualify, REPORT_SHA256, KERNEL
from gdn_dual_state_scope import scoped_copy


FILES = ('gdn_dual_state_experiment.py', 'gdn_dual_state_comparison.py', 'gdn_dual_state_gate.py',
    'gdn_dual_state_scope.py', 'gdn_dual_state_copy.py')
...
def run_loaded_requests(operations, generator, model, collectives, tokenizer, pages, kv_cache, parameters,
    ...
    if (any(os.environ.get(name) != '1' for name in ('QWEN_GDN_DUAL_STATE_COPY',
        ...
        raise ValueError('Explicit allocated unprofiled winning 4K copy comparison required')
    directory = Path(__file__).parent
    evidence = qualify(directory / 'dual-state-evidence', directory, os.environ['TT_METAL_HOME'])
    ...
    def measure(*args, **kwargs):
        if len(calls) >= len(SCHEDULE):
            raise ValueError('Unexpected additional dual-state request')
        ...
        if (kwargs.get('audit_features') is not audited
            ...
            raise ValueError('Both copy arms must retain the complete winning T16 runtime')
        calls.append((enabled, audited))
        with (scoped_copy(evidence) if enabled else nullcontext()) as audit:
        ...
        if enabled and (not audit['restored'] or builds <= 0 or builds % 48
            ...
            raise ValueError('Every norm-prefetched target layer must execute the admitted copy kernel')
        result['gdn_dual_state'] = dict(shared_unpack=enabled, builds=builds,
    ...
    def finish(result, summarize_requests):
        ...
        result.update(dual_state_comparison=comparison, pp=None, committed_tg=None,
            ctx_tokens=4096, fresh_context_audit=True,
            scope='Same winning T16/HiFi4 runtime; GDN state-copy fan-out only')
    ...
    try:
        ...
        report['dual_state_sources'] = sources
        report['dual_state_sources_after'] = fingerprints()
        if (report['dual_state_sources_after'] != sources
                or qualify(directory / 'dual-state-evidence', directory, os.environ['TT_METAL_HOME']) != evidence):
