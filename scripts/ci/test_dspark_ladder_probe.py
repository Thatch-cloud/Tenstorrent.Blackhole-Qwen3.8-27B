        ...
        for context in PROBE.CONTEXTS:
            with self.subTest(context=context):
                ...
                def run():
                    ...
                    self.assertEqual(report['stage_coordinates'], dict(row=8, column=116) if context == 65536 else None)
                    self.assertFalse(report['value_diagnostics_enabled'])
                    self.assertEqual(report['output_recurrence'],
                        'native-l1-pack-accumulation-fp32' if context == 65536 else 'native-l1-pack-accumulation-bf16')
