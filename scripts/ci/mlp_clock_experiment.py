...
from mlp_clock_hardware import retained
from mlp_clock_projection import instrument_projection
from mlp_clock_report import validate


HARDWARE_SHA256 = 'b47176bc1397aef8bb9115faad2c0a745c729b0b18eec75c1480fb3616059c22'
HELPERS = ('mlp_clock_capture.py', 'mlp_clock_projection.py', 'mlp_clock_samples.py',
...
def candidate_class(directory, evidence):
    ...
    expected = json.loads((evidence / 'clock-hardware-sources.json').read_text())
    for name in HELPERS:
    ...
    module = ModuleType('qualified_mlp_clock_candidate')
...
def run_loaded_requests(operations, generator, model, collectives, tokenizer, pages, kv_cache, parameters,
    ...
    if (os.environ.get('QWEN_MLP_CLOCK_COMBINED') != '1'
    ...
    candidate = candidate_class(directory, directory / 'clock-evidence')
    source_names = (*HELPERS, 'mlp_clock_experiment.py', 'mlp_clock_combined.py', 'mlp_clock_hardware.py',
        'mlp_clock_report.py', 'fused_1d.py', 'fused_1d_input.cpp', 'fused_1d_weights.cpp')
    def fingerprints():
    ...
    def finish(result, summarize):
        ...
        result.update(pp=None, committed_tg=None, fresh_context_audit=True, ctx_tokens=4096,
            ...
            mlp_clock_combined=bank.summary())
        finished.append(True)

    try:
        progress('allocate_all_layer_clock_pages_before_request_warmup')
        captures = [ClockCapture(operations, model.mesh_device, owned) for unused in range(64)]
        ...
        def measure(*args, **kwargs):
            ...
            result['mlp_clock_combined'] = bank.summary()
            return result
        ...
        report['mlp_clock_sources'] = sources
        report['mlp_clock_sources_after'] = fingerprints()
        if report['mlp_clock_sources_after'] != sources:
