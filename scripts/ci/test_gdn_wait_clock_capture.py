...
from gdn_wait_clock import MAGIC, ZONES
...
class WaitCaptureTests(unittest.TestCase):
    def setUp(self):
        ...
        self.fixture.setUp()
        self.owned = []
        self.capture = WaitClockCapture(self.fixture.operations, self.fixture.mesh, self.owned)
    ...
    def test_invalid_token_and_changed_placement_rejected(self):
        for token in (True, -1, 16):
            with self.assertRaises(ValueError):
                WaitClockCapture(self.fixture.operations, self.fixture.mesh, [], token=token)
        self.capture.prepare()
        self.execute()
