...
def full_mask(context_rows):
    ...
    padded_keys = (live_keys + 31) // 32 * 32
...
def validate_mask(mask, context_rows):
    import torch

    expected = full_mask(context_rows)
...
def execute(operations, query, key, value, mask, *, context_rows, mask_validated=False):
    ...
    expected_keys = (context_rows + 7 + 31) // 32 * 32
...
def fixtures(context_rows):
    ...
    return [*patterns, padded, oldest, future]
