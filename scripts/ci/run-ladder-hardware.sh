...
cleanup() {
    ...
    trap - EXIT
    if [ -n "$container" ]; then
        timeout -k 5 20 docker inspect --format '{{json .State}}' "$container" > "$results/ladder-container-state.json" || true
...
container=$(docker create --network none --hostname qwen-experiment --add-host qwen-experiment:127.0.0.1 \
    ...
    -e TT_METAL_HOME=/opt/tt-metal -e TT_METAL_CACHE=/tmp/ladder-kernels -e MESH_DEVICE=P300 \
    ...
    --entrypoint /bin/bash "$image" /experiment-scripts/ci/ladder-hardware-suite.sh)
...
docker start -a "$container" 2>&1 | tee "$results/ladder-hardware-console.log"
