"""Matched complete requests; only GDN state-copy unpack/handoff work changes."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('gdn_dual_state', {}).get('shared_unpack'), value.get('instrumented_timing'))
            for value in requests] != list(SCHEDULE):
        raise ValueError('Two fresh audits followed by complete unchanged/shared_unpack/shared_unpack/unchanged requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['gdn_dual_state']['shared_unpack'] is enabled]) for name, enabled in (('unchanged', False), ('shared_unpack', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(unchanged_tg=baseline, shared_unpack_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['shared_unpack']['committed_tg'] / arms['unchanged']['committed_tg'] - 1),
