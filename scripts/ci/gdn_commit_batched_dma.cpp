...
void kernel_main() {
    ...
    for (uint32_t batch = 0; batch < 384; batch += 16) {
    ...
    for (uint32_t batch = 0; batch < 640; batch += 16) {
        for (uint32_t lane = 0; lane < 8; ++lane) {
            ...
            if (prefix == 0) {
                noc_async_read(entry.get_noc_addr(page, 0), staged, 32);
                noc_async_read(entry.get_noc_addr(page, 512), staged + 512, 32);
            } else {
                noc_async_read(history.get_noc_addr(page, offset), staged, 32);
                noc_async_read(history.get_noc_addr(page, offset + 512), staged + 512, 32);
            }
        }
        noc_async_read_barrier();
        for (uint32_t lane = 0; lane < 8; ++lane) {
            ...
            for (uint32_t face = 0; face < 2; ++face) {
                const auto source = reinterpret_cast<volatile const uint32_t*>(staged + face * 512);
