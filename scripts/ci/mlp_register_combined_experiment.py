...
from mlp_register_combined_comparison import SCHEDULE, summarize
...
def run_loaded_requests(operations, generator, model, collectives, tokenizer, pages, kv_cache, parameters,
    ...
    if (os.environ.get('QWEN_MLP_REGISTER_EPILOGUE') != '1'
    ...
    runtime_root = Path(os.environ.get('TT_METAL_HOME', '/opt/tt-metal'))
    evidence = qualify(directory, directory / 'register-epilogue-evidence', runtime_root=runtime_root)
    spec = importlib.util.spec_from_file_location('qualified_register_epilogue_candidate',
    ...
    files = ('mlp_register_epilogue_gate.py', 'mlp_register_combined_comparison.py', 'mlp_register_combined_experiment.py',
        'mlp_register_epilogue.py', 'mlp_rounding_policy.py', 'mlp_weight_pipeline_gate.py',
        'mlp_weight_pipeline.py', 'mlp-register-epilogue-candidate/fused_1d.py',
        'mlp-register-epilogue-candidate/fused_1d_input.cpp', 'mlp-register-epilogue-candidate/fused_1d_weights.cpp')
    def fingerprints():
    ...
    def measure(*args, **kwargs):
        ...
        result['register_epilogue'] = dict(register_resident=enabled,
            simulator_report_sha256=REPORT_SHA256 if enabled else None)
        return result

    def route(value, arm):
        identity = value.get('register_epilogue', {})
        ...
        if (type(enabled) is not bool or arm != 'publication'
            ...
            raise ValueError('Explicit source-bound register-epilogue identity required')
    ...
    def finish(result, summarize_requests):
        ...
        result.update(register_epilogue_comparison=comparison, pp=None, committed_tg=None,
            ctx_tokens=4096, fresh_context_audit=True,
            scope='Same winning T16 recipe with unchanged versus register_resident register-resident rounded epilogue')
    ...
    try:
        ...
        report['register_epilogue_sources'] = sources
        report['register_epilogue_sources_after'] = fingerprints()
        if report['register_epilogue_sources_after'] != sources or qualify(
                directory, directory / 'register-epilogue-evidence', runtime_root=runtime_root) != evidence:
            raise ValueError('Register-epilogue qualification or sources changed during execution')
