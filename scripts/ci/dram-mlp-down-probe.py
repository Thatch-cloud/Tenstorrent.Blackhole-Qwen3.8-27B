"""Simulator T16 complete local MLP; synthetic weights, no collective or speed claim."""
...
def main():
    ...
    files = ('dram-mlp-down-probe.py', 'dram_mlp_down.py', 'dram_sharded_projection.py',
    ...
    try:
        ...
        for name, (inner, width, cores, dtype, activated) in PROJECTIONS.items():
            ...
            sharded[name] = (keep(ttnn.to_memory_config(weights[name], configs[name]['weights']))
                if name == 'down' else weights[name])
            ...
            if name != 'down':
                configs[name] = dict(program=programs[name])
            del host
