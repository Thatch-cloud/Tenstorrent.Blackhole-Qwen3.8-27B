...
void kernel_main() {
    ...
    auto assembled = reinterpret_cast<volatile uint32_t*>(scratch + 4096);
    for (uint32_t task = worker; task < 16 * key_tiles; task += workers) {
        ...
        for (uint32_t key = 0; key < 32; key++) {
            for (uint32_t column = 0; column < width_tiles; column++) {
                ...
                auto source = reinterpret_cast<volatile const uint32_t*>(scratch);
        ...
        noc_async_write_tile(task, output, scratch + 4096);
