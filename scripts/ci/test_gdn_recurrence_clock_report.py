...
from gdn_recurrence_clock_report import MODES, validate


def fixture():
    report = dict(passed=True, closed_cleanly=True, norm_unchanged=True, state_math_unchanged=True,
        shared_qk_preparation=True, recurrence_clock=True, missing_execution_rejected=True,
    ...
    report['recurrence_clock_samples'] = [dict(label=mode, poisoned_before_execution=True,
...
class RecurrenceReportTests(unittest.TestCase):
    ...
    def test_incomplete_numeric_or_sample_matrix_rejected(self):
        for field in ('checks', 'immutable_checks', 'recurrence_clock_samples'):
            report = fixture()
            report[field].pop()
        ...
        for field, value in (('token', 9), ('chip', 1), ('duration_cycles', 21),
            ...
            report = fixture()
            report['recurrence_clock_samples'][0]['samples'][0][field] = value
            with self.assertRaises(ValueError):
                validate(report)
    ...
    def test_missing_poison_source_drift_or_speed_claim_rejected(self):
        for field, value in (('missing_execution_rejected', False),
            ...
            with self.assertRaises(ValueError):
                validate(report)
        report = fixture()
        report['recurrence_clock_samples'][1] = deepcopy(report['recurrence_clock_samples'][0])
        with self.assertRaises(ValueError):
            validate(report)
