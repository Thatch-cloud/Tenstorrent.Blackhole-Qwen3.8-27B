...
def summarize_requests(requests, *, arm_key='norm_batch'):
    ...
    if arm_key not in ('norm_batch', 'attention_replay', 'attention_wide', 'lookup_cap', 'sampling_links', 'native_sampling_rows', 'mtp_short_attention', 'mtp_reuse_cache', 'mtp_kv_only', 'mtp_device_chain'):
    ...
    for entry in requests:
        ...
        if arm_key in ('native_sampling_rows', 'mtp_short_attention', 'mtp_reuse_cache', 'mtp_kv_only', 'mtp_device_chain'):
            mtp = entry.get('mtp', {})
            short = arm_key == 'mtp_short_attention'
            reuse = arm_key == 'mtp_reuse_cache'
            chained = arm_key == 'mtp_device_chain'
            bounded = short or reuse or arm_key == 'mtp_kv_only' or chained
            ...
            if arm_key == 'mtp_kv_only' or chained:
                cache = mtp.get('valid_cache', {})
                digests = cache.get('keys', []) + cache.get('values', [])
                if (mtp.get('kv_only_repair') is not (True if chained else entry[arm_key])
                ...
                if chained:
                    setup = mtp.get('chain_setup_ms', 0)
                    if (mtp.get('device_chain') is not entry[arm_key] or type(setup) not in (int, float)
                            or not math.isfinite(setup) or setup < 0 or (setup > 0) is not entry[arm_key]):
                        raise ValueError('Explicit device chain and measured chain preparation required')
            if reuse:
        ...
        if arm_key in ('native_sampling_rows', 'mtp_short_attention', 'mtp_reuse_cache', 'mtp_kv_only', 'mtp_device_chain') and mtp_setup <= 0:
