...
from dspark_normalization_stage_gate import qualify
...
def main():
    ...
    qualify(directory, directory / 'dspark-normalization-direct-stage.json')
    ...
    with normalization_stage_scope(), staging_scope(), boundary_scope(os.environ['TT_METAL_HOME']), \
    ...
    report['candidate'] = 'normalization-direct-staging'
    report['candidate_sources'].update({name: hashlib.sha256((directory / name).read_bytes()).hexdigest()
        for name in ('dspark_direct_fp32_stage.py', 'dspark_normalization_stage_gate.py', 'dspark_normalization_direct_stage.py',
