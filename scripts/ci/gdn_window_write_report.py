...
def validate_route(request, arm):
    ...
    identity = request.get('gdn_window_write', {})
    enabled, hits = identity.get('overlap'), identity.get('hits')
...
def validate(report):
    ...
    for before, after in (('sources', 'sources_after'), ('native_sources', 'native_sources_after'),
            ('gdn_window_write_sources', 'gdn_window_write_sources_after')):
    ...
    if comparison != report.get('gdn_window_write_comparison'):
