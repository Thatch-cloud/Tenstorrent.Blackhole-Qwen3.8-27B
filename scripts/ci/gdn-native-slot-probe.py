"""Synthetic T16 native-slot versus compact recurrence; no hardware or speed qualification."""
...
from gdn_native_slot_recurrence import execute as native_slot
from gdn_vsplit import execute as compact


SOURCES = ('gdn-native-slot-probe.py', 'gdn_native_slot_recurrence.py', 'gdn_vsplit.py',
...
def main():
    ...
    def fixture(seed):
        ...
        return [torch.randn((2, 16, 5120), generator=generator).to(torch.bfloat16) * 0.1,
            torch.sigmoid(torch.randn((2, 16, 24), generator=generator)).to(torch.bfloat16),
            (-torch.rand((2, 16, 24), generator=generator)).to(torch.bfloat16), state,
            torch.randn((2, 16, 3072), generator=generator).to(torch.bfloat16),
            torch.ones((2, 1, 128), dtype=torch.bfloat16), state[[0, 8]].clone()]
    def upload(value):
    ...
    try:
        ...
        def run(candidate):
            if candidate:
                values = native_slot(ttnn, mesh, *inputs[:4], z=inputs[4], norm_w=inputs[5], root=root, experimental=True)
            else:
                values = compact(mesh, *inputs[:3], inputs[6], z=inputs[4], norm_w=inputs[5], root=root,
                    experimental=True, batch_norm=True, synchronize=False, output_memory=ttnn.L1_MEMORY_CONFIG)
            owned.extend(values)
            return values
        ...
        if len(report['checks']) != 42 or len(report['immutable_checks']) != 56:
