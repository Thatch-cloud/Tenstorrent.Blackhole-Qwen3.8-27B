"""Simulator-only local winner records; unchanged SFPU addition, no production token route."""
...
def execute_local_winners(operations, mesh, base, bias, step, retain, *, worker_limit=110):
    if os.environ.get('QWEN_SIM_ONLY') != '1' or not os.environ.get('TT_METAL_SIMULATOR'):
        raise ValueError('Compact local winners are simulator-only')
    ...
    output = retain(operations.empty((1, 1, workers, 8), dtype=operations.uint32, layout=operations.ROW_MAJOR_LAYOUT,
    ...
    for chip, tensors in enumerate(zip(*shards, strict=True)):
        ...
        reader = operations.KernelDescriptor(kernel_source=str(Path(__file__).with_name('compact_score_io.cpp')),
        ...
        compute = operations.KernelDescriptor(kernel_source=str(Path(__file__).with_name('compact_score_compute.cpp')),
