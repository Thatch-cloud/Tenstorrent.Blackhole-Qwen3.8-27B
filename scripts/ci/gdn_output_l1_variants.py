"""Matched complete requests changing only the GDN partial-output placement."""
...
from gdn_output_l1_gate import REPORT_SHA256


POLICIES = {'control': dict(PUBLICATION['publication']),
    'publication': dict(PUBLICATION['publication'], gdn_output_l1=True)}


def validate_route(value, arm):
    if arm not in POLICIES:
        raise ValueError('Unknown GDN output placement arm')
    publication_route(value, 'publication')
    evidence = value.get('gdn_output_l1')
    if arm == 'control':
        if evidence is not None:
            raise ValueError('Control must retain native output placement')
        return
    if (not isinstance(evidence, dict) or evidence.get('restored') is not True
        ...
        raise ValueError('Admitted and restored GDN output placement required')
