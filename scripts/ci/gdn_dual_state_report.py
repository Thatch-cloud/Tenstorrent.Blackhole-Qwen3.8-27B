"""Independently recompute combined GDN state-copy correctness and full-cycle TG."""
...
def validate_route(request, arm):
    ...
    identity = request.get('gdn_dual_state', {})
    enabled, builds = identity.get('shared_unpack'), identity.get('builds')
    ...
    if (arm != 'publication' or type(enabled) is not bool or type(builds) is not int
        ...
        raise ValueError('All norm-prefetched recurrence layers must use the admitted copy policy')
