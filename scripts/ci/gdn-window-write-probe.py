"""Four in-flight T16 window writes versus frozen control and CPU indexing."""
...
def candidate(mesh, projected, history):
    path = Path(__file__).parent / 'window-write-candidate/gdn_conv_windows.py'
    spec = importlib.util.spec_from_file_location('window_write_candidate', path)
...
def main():
    ...
    sources = (Path(__file__).name, 'window-write-candidate/gdn_conv_windows.py',
        'window-write-candidate/gdn_conv_windows.cpp', 'gdn_conv_windows.py',
    ...
    def verify(outputs, inputs, host, seed, mode):
        ...
        for arm, values in zip(('control', 'overlap'), outputs, strict=True):
    ...
    try:
        ...
        def run(native):
            values = candidate(mesh, inputs[0], inputs[5:]) if native else compact(mesh, inputs[0], inputs[5:])
            owned.extend(values)
            return values
        save('eager')
