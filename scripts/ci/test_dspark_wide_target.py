...
import dspark_wide_target as wide
...
class DSparkWideTargetTests(unittest.TestCase):
    ...
    def test_anchor_row_and_fourteen_masks_use_the_entire_requested_geometry(self):
        ...
        for anchor, position, proposals in ((True, 4096, 15), (-1, 4096, 15), (248320, 4096, 15), (1, 8193, 15), (1, 4096, 31)):
            with self.assertRaises(ValueError):
                wide.query_inputs(anchor, position, proposals)

    def test_fifteen_embeddings_gather_both_hidden_shards_then_pad_seventeen_rows(self):
        ...
        identifiers = SimpleNamespace(shape=(1, 15), dtype='uint32', layout='row', memory_config=lambda: 'dram')
        operations.reshape.return_value = tensor((1, 1, 15, 2560))
        operations.experimental.all_gather_async.return_value = tensor((1, 1, 15, 5120))
        operations.pad.return_value = tensor((1, 1, 32, 5120))
        with patch.object(wide, 'projection_links', return_value=4):
            output = wide.noise_embeddings(operations, target, mesh, collectives, identifiers, lambda value: value, proposals=15)
        ...
        self.assertEqual(operations.pad.call_args.args[1], [(0, 0), (0, 0), (0, 17), (0, 0)])
        operations.to_torch.assert_not_called()

    def test_complete_vocab_is_gathered_before_slicing_all_fifteen_rows_from_zero(self):
        operations = self.operations()
        ...
        operations.slice.return_value = tensor((1, 1, 15, 248320))
        operations.typecast.return_value = tensor((1, 1, 15, 248320), dtype='float32')
        with patch.object(wide, 'projection_links', return_value=4):
            output = wide.gather_logits(operations, mesh, MagicMock(), local, lambda value: value, proposals=15)
    ...
    def test_packed_tokens_preserve_row_zero_order_and_use_bounded_concat(self):
        ...
        values = [SimpleNamespace(shape=(1, 1, 1), dtype='uint32', layout='row',
            memory_config=lambda: 'dram', tokens=[index]) for index in range(15)]
        ...
        self.assertEqual(output.tokens, list(range(15)))
        self.assertEqual(operations.concat.call_count, 3)
        operations.to_torch.assert_not_called()
