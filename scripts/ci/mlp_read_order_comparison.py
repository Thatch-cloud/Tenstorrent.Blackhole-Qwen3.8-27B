"""Matched complete requests; only the T16 weight-reader issue order changes."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('weight_read_order', {}).get('staggered'), value.get('instrumented_timing'))
            for value in requests] != list(SCHEDULE):
        raise ValueError('Two fresh audits followed by complete unchanged/staggered/staggered/unchanged requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['weight_read_order']['staggered'] is enabled]) for name, enabled in (('unchanged', False), ('staggered', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(unchanged_tg=baseline, staggered_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['staggered']['committed_tg'] / arms['unchanged']['committed_tg'] - 1),
