...
preserve_metadata() {
    ...
    if [ -f /experiment/results/dspark-verifier-profile-hardware.json ]; then
        cp /experiment/results/dspark-verifier-profile-hardware.json "$output/request.json"
    fi
}
...
arguments=(--request --profile-verifier --checkpoint /dspark/model.safetensors --config /dspark/config.json
    --output /experiment/results/dspark-verifier-profile-hardware.json)
