"""Direct FP32 staging candidate under the combined draft and target simulator gates."""
...
def main():
    ...
    report.update(candidate='direct-fp32-staging', performance_qualified=False,
        ...
        candidate_sources={name: hashlib.sha256((directory / name).read_bytes()).hexdigest()
            for name in ('dspark_score_bitwise.py', 'dspark_score_smoke_geometry.py',
                ...
                'dspark_sum_sfpu_build.py', 'dspark_mask_bits.py', 'dspark_attention_header_boundary.py', 'dspark_direct_fp32_stage.py', Path(__file__).name)})
