...
import dspark_t32_prepared as prepared
...
class PreparedProposalTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.device = SimpleNamespace(closed=False, position=32, max_drafts=31, operations=self.operations,
    ...
    def execute(self, device, inputs, history, retain):
        ...
        tokens = torch.arange(anchor + 1, anchor + 32).reshape(1, 1, 31, 1)
        return dict(normalized=FakeTensor(torch.full((1, 1, 32, 5120), anchor, dtype=torch.bfloat16)),
            logits=FakeTensor(torch.full((1, 1, 31, 32), anchor, dtype=torch.float32)), tokens=FakeTensor(tokens))
    ...
    def test_audit_compares_complete_fixed_layout_outputs_on_both_replicas(self):
        ...
        self.assertEqual(proposal.propose(20, 31), tuple(range(21, 52)))
    ...
    def test_all_five_layers_use_physical_capacity_but_pass_actual_absolute_rotary_tables(self):
        ...
        for index, call in enumerate(layer.call_args_list):
            self.assertEqual(call.kwargs, dict(position=64, proposals=31, mask_validated=True))
        ...
        self.assertEqual(noise.call_args.kwargs['proposals'], 31)
