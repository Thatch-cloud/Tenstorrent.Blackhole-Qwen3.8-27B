"""Stage synthetic normalized Q/K double buffering with complete prefix-state replay checks."""
...
def adapt(source):
    source = replace_once(source, 'from gdn_shared_qk_pipeline import build as build_pipeline',
        'from gdn_qk_double_buffer import build as build_pipeline, BUILD_RECORDS')
    source = replace_once(source,
        'rows=16, norm_unchanged=True, state_math_unchanged=True, shared_qk_preparation=True)',
        'rows=16, norm_unchanged=True, state_math_unchanged=True, shared_qk_preparation=True,\n'
        '        qk_double_buffer=True, generated_kernels=BUILD_RECORDS)')
...
def main():
    ...
    helper = Path(__file__).with_name('gdn_qk_double_buffer.py').read_bytes()
    (scripts / 'gdn_qk_double_buffer.py').write_bytes(helper)
