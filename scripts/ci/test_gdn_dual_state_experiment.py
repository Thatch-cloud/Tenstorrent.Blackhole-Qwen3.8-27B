...
import gdn_dual_state_experiment as experiment
from dspark_request_experiment import summarize
from test_gdn_dual_state_comparison import fixture


class DualStateExperimentTests(unittest.TestCase):
    def test_all_six_requests_keep_norm_prefetch_and_restore_routes(self):
        with TemporaryDirectory() as temporary:
            ...
            environment = {name: '1' for name in ('QWEN_GDN_DUAL_STATE_COPY', 'QWEN_FROZEN_COMBINED_RUNTIME',
            ...
            with patch.dict('sys.modules', modules), patch.dict('os.environ', environment, clear=True), \
                    patch.object(experiment, '__file__', str(directory / 'gdn_dual_state_experiment.py')), \
                    patch.object(experiment, 'qualify', return_value={}), patch.object(experiment, 'scoped_copy', scope):
            ...
            self.assertEqual(report['dual_state_comparison']['arms']['shared_unpack']['committed_tg'], 120)
            self.assertEqual(report['dual_state_sources'], report['dual_state_sources_after'])
