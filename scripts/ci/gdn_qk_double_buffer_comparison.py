"""Matched complete requests; only GDN normalized Q/K ring capacity changes."""
...
def summarize(requests, summarize_requests, validate_audit, validate_route):
    if [(value.get('gdn_qk_double_buffer', {}).get('double_buffered_qk'), value.get('instrumented_timing'))
            for value in requests] != list(SCHEDULE):
        raise ValueError('Two fresh audits followed by complete unchanged/double_buffered_qk/double_buffered_qk/unchanged requests required')
    ...
    arms = {name: summarize_requests([value for value in requests
        if value['gdn_qk_double_buffer']['double_buffered_qk'] is enabled]) for name, enabled in (('unchanged', False), ('double_buffered_qk', True))}
    pairs = []
    for control, candidate in ((requests[2], requests[3]), (requests[5], requests[4])):
        ...
        pairs.append(dict(unchanged_tg=baseline, double_buffered_qk_tg=changed, change_percent=100 * (changed / baseline - 1)))
    return dict(arms=arms, pairs=pairs,
        committed_tg_change_percent=100 * (arms['double_buffered_qk']['committed_tg'] / arms['unchanged']['committed_tg'] - 1),
