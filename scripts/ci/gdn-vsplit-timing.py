...
def validate_matrix(fixtures):
    ...
    if ({(record['seed'], record['rows']) for record in fixtures} != expected
            ...
            or any(record.get('exact') is not True or record['timed_replays'] != 120 for record in fixtures)):
...
def main():
    ...
    path = Path('/experiment/results/gdn-vsplit-timing.json')
    report = dict(passed=False, fixtures=[], prerequisite_run=34046231437,
        ...
        sources={name: hashlib.sha256(Path(__file__).with_name(name).read_bytes()).hexdigest()
                 for name in ('gdn_vsplit_prepared.py', 'gdn_vsplit.py', 'gdn_multitoken.py')})
    mesh = None
            ...
            for rows in (1, 2, 4, 8, 16, 32):
                ...
                try:
                    ...
                    for index in range(rows):
                        tokens = [upload(value[:, index:index + 1]) for value in values]
                        output = None
                        try:
                    ...
                    expected_outputs = [torch.cat(values, dim=0) for values in expected_outputs]
                    expected_states = [torch.cat(values, dim=0) for values in expected_states]
                    prepared = PreparedVSplit(mesh, *packed[:3], initial, z=packed[3], norm_w=norm_w,
                        experimental=True, output_memory=ttnn.L1_MEMORY_CONFIG)
