...
from gdn_wait_clock_experiment import FILES
from gdn_wait_clock_gate import retained
from gdn_wait_clock_program import instrument_pipeline
...
def adapt(sources):
    ...
    result['dspark-target-hardware.py'] = replace_once(result['dspark-target-hardware.py'],
        ...
        '            from gdn_wait_clock_experiment import run_loaded_requests')
    result['run-dspark-hardware.sh'] = replace_once(result['run-dspark-hardware.sh'],
        '    -e "QWEN_DSPARK_MODE=$mode"',
        '    -e "QWEN_GDN_WAIT_CLOCK_COMBINED=${QWEN_GDN_WAIT_CLOCK_COMBINED:-0}" \\\n'
        '    -e "QWEN_DSPARK_MODE=$mode"')
...
def main():
    ...
    if options.manifest.exists() or (scripts / 'wait-clock-evidence').exists():
    ...
    result['gdn_wait_clock_pipeline.py'] = instrument_pipeline(
    ...
    shutil.copytree(options.evidence, scripts / 'wait-clock-evidence')
    retained(scripts / 'wait-clock-evidence', scripts)
