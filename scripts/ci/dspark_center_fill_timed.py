...
from dspark_center_fill_request_gate import qualify as qualify_numerical
from dspark_center_fill_screen import summarize_screen


SCREEN_RUN = 34841889707
SCREEN_SHA256 = 'a666b0fb81a6d6754c72357d66aa1c10b39d6941b06a561381c0e6ef7ea0c0f2'


def qualify(directory, report_directory=None):
    reports = Path(directory if report_directory is None else report_directory)
    admission = qualify_numerical(directory, Path(directory) / 'dspark-center-tile-fill-hardware.json')
    ...
    sources = comparison['center_fill_sources']
    for name in ('dspark_direct_fp32_stage.py', 'dspark_normalization_direct_stage.py', 'dspark_center_tile_fill.py', 'dspark_center_fill_request_gate.py', 'dspark_center_fill_screen.py'):
