...
class NativeSourcesTests(unittest.TestCase):
    def exercise(self, *, corrupt_download=False, corrupt_existing=False, invalid_report=False, register=False,
                 retained=False, corrupt_retained=False):
        ...
        with TemporaryDirectory() as temporary:
            ...
            retained_root = root / 'retained' if retained else None
            if retained:
                for name in sources.PATHS[:3]:
                    target = retained_root / name
                    target.parent.mkdir(parents=True, exist_ok=True)
                    target.write_bytes(b'wrong' if corrupt_retained else payload)
            if corrupt_existing:
            ...
            with patch.object(sources, 'REPORT_SHA256', 'invalid' if invalid_report else hashlib.sha256(raw).hexdigest()), \
                ...
                if corrupt_download or corrupt_existing or invalid_report or corrupt_retained:
                    with self.assertRaises(ValueError):
                        sources.restore(report, destination, register_epilogue=register, retained_root=retained_root)
                    self.assertFalse((destination / sources.PATHS[-1]).exists())
                    if corrupt_existing or invalid_report or corrupt_retained:
                        download.assert_not_called()
                    return
                result = sources.restore(report, destination, register_epilogue=register, retained_root=retained_root)
                self.assertEqual(download.call_count, (9 if register else 7) - (3 if retained else 0))
    ...
    def test_runner_exported_kernels_do_not_require_upstream_download(self):
        self.exercise(register=True, retained=True)

    def test_corrupt_retained_kernel_is_rejected_without_download_fallback(self):
        self.exercise(retained=True, corrupt_retained=True)
